...
cp $1/regression/conf/conf_launcher_blackscholes.txt $1/run/esesc.conf
$1/main/esesc > $1/run/blackscholes.input 
$2/conf/scripts/report.pl -last > $1/regression/report/report_launcher_blackscholes.txt

for f in "launcher_fft" "memkernels_stream" "memkernels_stream_omp"  
do
   g='conf_'$f'.txt'
   cp $1/regression/conf/$g $1/run/esesc.conf
   $1/main/esesc
   h='report_'$f'.txt' 
   $2/conf/scripts/report.pl -last > $1/regression/report/$h
done

mv $1/regression/temp.conf $1/run/esesc.conf 
rm -r $1/regression/conf/ 
python $2/regression/reg-Extract.py $1 $2
