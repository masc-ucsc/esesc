...
void MCBackend::compute()
{
  ...
  area.set_area((2.7927*log(mcp.peakDataTransferRate)-19.862)/2.0*mcp.dataBusWidth/128.0*(l_ip.F_sz_um/0.09)*mcp.num_channels*1e6);//um^2
  ...
  C_MCB = mc_power/20e6/32/1.3/1.3*l_ip.F_sz_um/0.13;// Normalize from .13 um process where vdd=1.3. 10% memory traffic to the peak bw
//  refresh_period = 0.064;//64ms
 // refresh_freq =1.0/(refresh_period/pow(2.0,max_row_addr_width));
  ...
  power.readOp.dynamic += C_MCB*g_tp.peri_global.Vdd*g_tp.peri_global.Vdd*(mcp.dataBusWidth+mcp.addressBusWidth)*(1+1/num_bits_per_ecc_b_);//per access energy in memory controller
  power.readOp.leakage = area.get_area() *(g_tp.scaling_factor.core_tx_density/10)*cmos_Isub_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;//unit W
  power.readOp.gate_leakage = area.get_area() *(g_tp.scaling_factor.core_tx_density/10)*cmos_Ig_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;//unit W
  power_t = power;
}
...
void MCBackend::computeEnergy(bool is_tdp)
{
	...
	if (is_tdp)
    {
			//eka
			//power.reset();
		  power.readOp.dynamic	= (stats_t.readAc.access*power_t.readOp.dynamic+
...
void MCPHY::compute()
{
  ...
  area.set_area((6.4323*log(mcp.peakDataTransferRate)-48.134)*mcp.dataBusWidth/128.0*(l_ip.F_sz_um/0.09)*mcp.num_channels*1e6);//TODO:/2
