...
Htree2::Htree2(enum Wire_type wire_model, double mat_w, double mat_h, int a_bits, int d_inbits, int search_data_in, int d_outbits,
               int search_data_out, int bl, int wl, enum Htree_type htree_type, bool uca_tree_, bool search_tree_,
...
void Htree2::input_nand(double s1, double s2, double l_eff) {
  ...
  power.readOp.dynamic +=
      ...
      (2 * drain_C_(pton_size * nsize * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
       drain_C_(nsize * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) + 2 * gate_C(s2 * (min_w_nmos + min_w_pmos), 0)) *
  ...
  power.readOp.leakage +=
      (wire_bw * cmos_Isub_leakage(min_w_nmos * (nsize * 2), min_w_pmos * nsize * 2, 2, nand)) * deviceType->Vdd;
  power.readOp.gate_leakage +=
      (wire_bw * cmos_Ig_leakage(min_w_nmos * (nsize * 2), min_w_pmos * nsize * 2, 2, nand)) * deviceType->Vdd;
}
...
void Htree2::output_buffer(double s1, double s2, double l_eff) {
  ...
  double s_eff = // stage eff of a repeater in a wire
      (gate_C(s2 * (min_w_nmos + min_w_pmos), 0) + w1.wire_cap(l_eff * 1e-6)) / gate_C(s2 * (min_w_nmos + min_w_pmos), 0);
  ...
  power.readOp.dynamic += 0.5 *
                          (2 * drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                           drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) + gate_C(tr_size * (min_w_pmos), 0)) *
                          deviceType->Vdd * deviceType->Vdd;
  ...
  power.readOp.dynamic += 0.5 *
                          (drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                           drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) + gate_C(size * (min_w_nmos + min_w_pmos), 0)) *
                          deviceType->Vdd * deviceType->Vdd;

  power.searchOp.dynamic +=
      0.5 *
      (drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) + drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
  ...
  power.readOp.dynamic +=
      0.5 *
      (drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) + 2 * drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
  ...
  power.readOp.dynamic +=
      0.5 *
      ((drain_C_(tr_size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) + drain_C_(tr_size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def)) *
           2 +
       gate_C(s1 * (min_w_nmos + min_w_pmos), 0)) *
      deviceType->Vdd * deviceType->Vdd;
  ...
  if(uca_tree) {
    power.readOp.leakage += cmos_Isub_leakage(min_w_nmos * tr_size * 2, min_w_pmos * tr_size * 2, 1, inv) * deviceType->Vdd *
                            wire_bw; /*inverter + output tr*/
    power.readOp.leakage +=
        cmos_Isub_leakage(min_w_nmos * size * 3, min_w_pmos * size * 3, 2, nand) * deviceType->Vdd * wire_bw; // nand
    power.readOp.leakage +=
        cmos_Isub_leakage(min_w_nmos * size * 3, min_w_pmos * size * 3, 2, nor) * deviceType->Vdd * wire_bw; // nor

    power.readOp.gate_leakage += cmos_Ig_leakage(min_w_nmos * tr_size * 2, min_w_pmos * tr_size * 2, 1, inv) * deviceType->Vdd *
                                 wire_bw; /*inverter + output tr*/
    power.readOp.gate_leakage +=
        cmos_Ig_leakage(min_w_nmos * size * 3, min_w_pmos * size * 3, 2, nand) * deviceType->Vdd * wire_bw; // nand
    power.readOp.gate_leakage +=
        cmos_Ig_leakage(min_w_nmos * size * 3, min_w_pmos * size * 3, 2, nor) * deviceType->Vdd * wire_bw; // nor
    // power.readOp.gate_leakage *=;
  } else {
    power.readOp.leakage += cmos_Isub_leakage(min_w_nmos * tr_size * 2, min_w_pmos * tr_size * 2, 1, inv) * deviceType->Vdd *
                            wire_bw; /*inverter + output tr*/
...
void Htree2::in_htree() {
  ...
  if(ndwl == ndbl) {
    ...
    // data traverse in a h link followed by a v link in a repeated fashion (similar to a balanced tree) until there are no h links
    // left
    hdist = ndwl / 2 - 1;
  ...
  if(uca_tree) {
    ht_temp = (mat_height * ndbl /* since uca_tree models interbank tree, mat_height => bank height */ +
               (add_bits + data_in_bits + data_out_bits + (search_data_in_bits + search_data_out_bits)) * hdist *
                   g_tp.wire_outside_mat.pitch) /
              2;
    len_temp = (mat_width * ndwl /* mat width => bank width */ +
                (add_bits + data_in_bits + data_out_bits + (search_data_in_bits + search_data_out_bits)) *
                    g_tp.wire_outside_mat.pitch * vdist) /
               2;
  } else {
    if(ndwl == ndbl) {
    ...
    len_temp = (mat_width * ndwl / 2 + (add_bits + data_in_bits + data_out_bits + (search_data_in_bits + search_data_out_bits)) *
                                           g_tp.wire_outside_mat.pitch * vdist) /
               2;
  }
