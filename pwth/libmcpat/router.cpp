...
double Router::crossbar_inpline() {
  return (Cw3(O * flit_size * wt) + O * transmission_buf_inpcap() + gate_cap(NTid) + gate_cap(PTid) + diff_cap(NTid, 0, 1) +
          diff_cap(PTid, 1, 1));
}

double Router::crossbar_outline() {
  return (Cw3(I * flit_size * ht) + I * transmission_buf_outcap() + gate_cap(NTod) + gate_cap(PTod) + diff_cap(NTod, 0, 1) +
          diff_cap(PTod, 1, 1));
}

double Router::crossbar_ctrline() {
  return (Cw3(0.5 * O * flit_size * wt) + flit_size * transmission_buf_ctrcap() + diff_cap(NTi, 0, 1) + diff_cap(PTi, 1, 1) +
...
void Router::buffer_stats() {
  ...
  dyn_p.num_c_subarray                            = (int)flit_size;
  ...
  dyn_p.cell.h =
      g_tp.sram.b_h + 2 * g_tp.wire_outside_mat.pitch * (dyn_p.num_wr_ports + dyn_p.num_rw_ports - 1 + dyn_p.num_rd_ports);
  dyn_p.cell.w = g_tp.sram.b_w +
                 2 * g_tp.wire_outside_mat.pitch *
                     (dyn_p.num_rw_ports - 1 + (dyn_p.num_rd_ports - dyn_p.num_se_rd_ports) + dyn_p.num_wr_ports) +
...
void Router::get_router_power() {
  ...
  //  arb_stats();
  power.readOp.dynamic = (I * (buffer.power.readOp.dynamic + buffer.power.writeOp.dynamic) + I * crossbar.power.readOp.dynamic +
                          I * arbiter.power.readOp.dynamic) *
                         M;
