 ...
 router(0)
{
  ...
  */
  ...
  router      = new Router(nocdynp.flit_size,
		         ...
		         nocdynp.input_ports,nocdynp.output_ports);
  //router->print_router();
...
void NoC::displayEnergy(uint32_t indent,int plevel,bool is_tdp)
{
  ...
  if (is_tdp)
  {
    ...
    if (plevel >2){
      ...
      cout << indent_str<< indent_str_next << "TDP Dynamic = " <<(router->buffer.power.readOp.dynamic + router->buffer.power.writeOp.dynamic)
        *nocdynp.input_ports*M*nocdynp.clockRate << " W" << endl;
      cout << indent_str<< indent_str_next << "Subthreshold Leakage = " << router->buffer.power.readOp.leakage*nocdynp.input_ports <<" W" << endl;
      ...
      cout << indent_str<< indent_str_next << "TDP Dynamic = " << router->crossbar.power.readOp.dynamic*nocdynp.clockRate*nocdynp.input_ports*M << " W" << endl;
      ...
      cout << indent_str<< indent_str_next << "TDP Dynamic = " << router->arbiter.power.readOp.dynamic*nocdynp.clockRate*nocdynp.input_ports*M  << " W" << endl;
  ...
  {
    //		cout << indent_str_next << "Instruction Fetch Unit    TDP Dynamic = " << ifu->rt_power.readOp.dynamic*clockRate << " W" << endl;
    ...
    //		cout << indent_str_next << "Load Store Unit   TDP Dynamic = " << lsu->rt_power.readOp.dynamic*clockRate  << " W" << endl;
    ...
    //		cout << indent_str_next << "Memory Management Unit   TDP Dynamic = " << mmu->rt_power.readOp.dynamic*clockRate  << " W" << endl;
    ...
    //		cout << indent_str_next << "Execution Unit   TDP Dynamic = " << exu->rt_power.readOp.dynamic*clockRate  << " W" << endl;
