...
// eka, TODO: remove this and make sure NTHREADS is passed
// through makefile
#ifndef NTHREADS
#define NTHREADS 2
#endif
...
#if NEW_BOOST
template <class Archive> void min_values_t::serialize(Archive &ar, unsigned int version) {
  //////////////////////// FIXME //////////////////////
  // ar & min_delay;
  // ar & min_dyn;
  // ar & min_leakage;
      ...
      for(uint32_t iter = tid; iter < niter; iter += nthreads) {
        ...
        for(unsigned int Ndsam_lev_1 = 1; Ndsam_lev_1 <= MAX_COL_MUX; Ndsam_lev_1 *= 2) {
          for(unsigned int Ndsam_lev_2 = 1; Ndsam_lev_2 <= MAX_COL_MUX; Ndsam_lev_2 *= 2) {
            // for debuging
            if(g_ip->force_cache_config && is_tag == true) {
              wr          = g_ip->wt;
              Ndwl        = g_ip->ndwl;
              Ndbl        = g_ip->ndbl;
              Nspd        = g_ip->nspd;
            ...
            if(is_tag == true) {
              is_valid_partition = calculate_time(is_tag, pure_ram, pure_cam, Nspd, Ndwl, Ndbl, Ndcm, Ndsam_lev_1, Ndsam_lev_2,
                                                  tag_arr.back(), 0, NULL, NULL, is_main_mem);
            }
            ...
            if(is_tag == false || g_ip->fully_assoc) {
              is_valid_partition = calculate_time(is_tag /*false*/, pure_ram, pure_cam, Nspd, Ndwl, Ndbl, Ndcm, Ndsam_lev_1,
                                                  Ndsam_lev_2, data_arr.back(), 0, NULL, NULL, is_main_mem);
            }
...
bool calculate_time(bool is_tag, int pure_ram, bool pure_cam, double Nspd, unsigned int Ndwl, unsigned int Ndbl, unsigned int Ndcm,
  ...
  if(flag_results_populate) { // For the final solution, populate the ptr_results data structure  -- TODO: copy only necessary
                              // variables
  } else {
    ...
    if(g_ip->ed == 1) {
      ...
      if(v) {
        cost = (d * ((niter)->access_time / minval->min_delay) + c * ((niter)->cycle_time / minval->min_cyc) +
                ...
                lp * ((niter)->power.readOp.leakage / minval->min_leakage) + a * ((niter)->area / minval->min_area));
...
void filter_tag_arr(const min_values_t *min, list<mem_array *> &list) {
  double cost = BIGNUM;
  double cur_cost;
  double wt_delay = g_ip->delay_wt, wt_dyn = g_ip->dynamic_power_wt, wt_leakage = g_ip->leakage_power_wt,
         wt_cyc = g_ip->cycle_time_wt, wt_area = g_ip->area_wt;
  mem_array *res = NULL;

  if(list.empty() == true) {
    cout << "ERROR: no valid tag organizations found(1)" << endl;
    exit(1);
  }
  ...
  while(list.empty() != true) {
    ...
    if(v) {
      cur_cost = wt_delay * (list.back()->access_time / min->min_delay) +
                 ...
                 wt_area * (list.back()->area / min->min_area) + wt_cyc * (list.back()->cycle_time / min->min_cyc);
    } else {
      cur_cost = BIGNUM;
    }
    if(cur_cost < cost) {
