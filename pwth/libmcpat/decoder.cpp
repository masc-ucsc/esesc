...
Decoder::Decoder(int _num_dec_signals, bool flag_way_select, double _C_ld_dec_out, double _R_wire_dec_out, bool fully_assoc_,
...
void Decoder::compute_widths() {
  ...
  if(exist) {
    ...
    num_gates = logical_effort(num_gates_min, num_in_signals == 2 ? gnand2 : gnand3, F, w_dec_n, w_dec_p, C_ld_dec_out,
                               p_to_n_sz_ratio, is_dram, is_wl_tr, g_tp.max_w_nmos_dec);
  }
}
...
void PredecBlk::compute_widths() {
  ...
  if(flag_L2_gate) {
    ...
    if((flag_two_unique_paths) ||
       (number_inputs_L1_gate == 2)) { // Whenever flag_two_unique_paths is true, it means first level of decoder employs
      ...
      number_gates_L1_nand2_path = logical_effort(min_number_gates_L1, gnand2, F, w_L1_nand2_n, w_L1_nand2_p, c_load_nand2_path,
                                                  p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
    }

    // Now find widths of gates along path in which first gate is a NAND3
    if((flag_two_unique_paths) ||
       (number_inputs_L1_gate == 3)) { // Whenever flag_two_unique_paths is TRUE, it means first level of decoder employs
      ...
      number_gates_L1_nand3_path = logical_effort(min_number_gates_L1, gnand3, F, w_L1_nand3_n, w_L1_nand3_p, c_load_nand3_path,
                                                  p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
    }
    ...
    if(number_inputs_L1_gate == 2) {
      ...
      number_gates_L1_nand2_path = logical_effort(min_number_gates_L1, gnand2, F, w_L1_nand2_n, w_L1_nand2_p, C_ld_predec_blk_out,
                                                  p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
      ...
      number_gates_L1_nand3_path = logical_effort(min_number_gates_L1, gnand3, F, w_L1_nand3_n, w_L1_nand3_p, C_ld_predec_blk_out,
                                                  p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
    }
  }
}
  ...
  if(exist) {
    ...
    if(flag_L2_gate) {
      ...
      for(i = 1; i < number_gates_L2 - 1; ++i) {
        ...
        c_intrinsic =
            drain_C_(w_L2_p[i], PCH, 1, 1, g_tp.cell_h_def, is_dram_) + drain_C_(w_L2_n[i], NCH, 1, 1, g_tp.cell_h_def, is_dram_);
        tf         = rd * (c_intrinsic + c_load);
        this_delay = horowitz(inrisetime_nand2_path, tf, 0.5, 0.5, RISE);
        delay_nand2_path += this_delay;
        inrisetime_nand2_path = this_delay / (1.0 - 0.5);
  ...
  if(flag_driver_exists) {
    ...
    if((blk->flag_two_unique_paths) || (blk->number_inputs_L1_gate == 2) || (number_input_addr_bits == 0) ||
      ...
      number_gates_nand2_path = logical_effort(min_number_gates, 1, F, width_nand2_path_n, width_nand2_path_p,
                                               c_load_nand2_path_out, p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
    }
    ...
    if((blk->flag_two_unique_paths) || (blk->number_inputs_L1_gate == 3) ||
      ...
      number_gates_nand3_path = logical_effort(min_number_gates, 1, F, width_nand3_path_n, width_nand3_path_p,
                                               c_load_nand3_path_out, p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
    }
  }
}
...
void PredecBlkDrv::compute_area() {
  ...
  if(flag_driver_exists) { // first check whether a predecoder block driver is needed
    ...
    gate_leak_nand2_path *=
        (num_buffers_driving_1_nand2_load + num_buffers_driving_2_nand2_load + num_buffers_driving_4_nand2_load);
...
void Driver::compute_widths() {
  ...
  number_gates =
      logical_effort(min_number_gates, 1, F, width_n, width_p, c_load, p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);
}
...
double Driver::compute_delay(double inrisetime) {
  ...
  for(i = 0; i < number_gates - 1; ++i) {
    ...
    c_intrinsic =
        drain_C_(width_p[i], PCH, 1, 1, g_tp.cell_h_def, is_dram_) + drain_C_(width_n[i], NCH, 1, 1, g_tp.cell_h_def, is_dram_);
    tf         = rd * (c_intrinsic + c_load);
    this_delay = horowitz(inrisetime, tf, 0.5, 0.5, RISE);
    delay += this_delay;
    inrisetime = this_delay / (1.0 - 0.5);
