...
void PipelinePower::compute_stage_vector()
{
  ...
  else
  {
	if (coredynp.core_ty==Inorder)
	{
		...
		/* pipe stage ID/EXE */
		num_piperegs += coredynp.decodeW*(coredynp.instruction_length + coredynp.pc_width + powers (2,coredynp.opcode_length)+ 2*coredynp.int_data_width)*coredynp.num_hthreads;
		/* pipe stage EXE/MEM */
		num_piperegs += coredynp.issueW*(3 * coredynp.arch_ireg_width + powers (2,coredynp.opcode_length) + 8*2*coredynp.int_data_width/*+2*powers (2,reg_length)*/);
		/* pipe stage MEM/WB the 2^opcode_length means the total decoded signal for the opcode*/
		num_piperegs += coredynp.issueW*(2*coredynp.int_data_width + powers (2,coredynp.opcode_length) + 8*2*coredynp.int_data_width/*+2*powers (2,reg_length)*/);
//		/* pipe stage 5/6 */
...
//		/* assume 50% extra in control signals (rule of thumb) */
	...
	else
	{
		...
		num_piperegs += coredynp.issueW*(coredynp.instruction_length  + coredynp.pc_width + 3*coredynp.phy_ireg_width)*coredynp.num_hthreads;
		...
		num_piperegs += coredynp.issueW*(3 * coredynp.phy_ireg_width + coredynp.pc_width + powers (2,coredynp.opcode_length)/*+2*powers (2,reg_length)*/);
		/* 2^opcode_length means the total decoded signal for the opcode*/
		num_piperegs += coredynp.issueW*(2*coredynp.int_data_width + powers (2,coredynp.opcode_length)/*+2*powers (2,reg_length)*/);
		...
		num_piperegs += coredynp.issueW*(coredynp.int_data_width + coredynp.v_address_width + powers (2,coredynp.opcode_length)/*+2*powers (2,reg_length)*/);//memory Opcode still need to be passed
 ...
 is_default(_is_default)
{
	...
	if (fu_type == FPU)
    	{
    	  ...
    	  leakage = area_t *(g_tp.scaling_factor.core_tx_density/10)*cmos_Isub_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;//unit W
    	  gate_leakage = area_t *(g_tp.scaling_factor.core_tx_density/10)*cmos_Ig_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;//unit W
    ...
    else
	{
	  ...
	  leakage = area_t *(g_tp.scaling_factor.core_tx_density/10)*cmos_Isub_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;//unit W
	  gate_leakage = area_t*(g_tp.scaling_factor.core_tx_density/10)*cmos_Ig_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;
...
UndiffCore::UndiffCore(const InputParameter *configure_interface, enum  Core_type core_ty_, double pipeline_stage_,double num_hthreads_,double issue_width_, bool embedded_,  bool _is_default)
:l_ip(*configure_interface),
 core_ty(core_ty_),
 ...
 is_default(_is_default)
{
	...
	if (embedded==false)
	{
		//Based on the results of polynomial/log curve fitting based on undifferentiated core of Niagara, Niagara2, Merom, Penyrn, Prescott die measurements
		if (core_ty==OOO)
	...
	power.readOp.leakage = undifferentiated_core*(core_tx_density/10)*cmos_Isub_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;//unit W
	power.readOp.gate_leakage = undifferentiated_core*(core_tx_density/10)*cmos_Ig_leakage(g_tp.min_w_nmos_, g_tp.min_w_nmos_*pmos_to_nmos_sizing_r, 1, inv)*g_tp.peri_global.Vdd/2;
