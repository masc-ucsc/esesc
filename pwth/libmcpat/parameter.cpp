...
void TechnologyParameter::display(uint32_t indent) {
  ...
  cout << indent_str << "dram_cell_I_off_worst_case_len_temp = " << setw(12) << dram_cell_I_off_worst_case_len_temp << " A/um"
       << endl;
...
RuntimeParameter::RuntimeParameter(bool is_first_call_) {
  is_first_call = is_first_call_;
}

bool RuntimeParameter::Is_first_call() {
  return (is_first_call);
}

void RuntimeParameter::Set_no_first_call() {
  is_first_call = false;
}
// to here by eka
...
DynamicParameter::DynamicParameter(bool is_tag_, int pure_ram_, int pure_cam_, double Nspd_, unsigned int Ndwl_, unsigned int Ndbl_,
  ...
  if(fully_assoc || pure_cam) { // fully-assocative cache -- ref: CACTi 2.0 report
  ...
  if(!(fully_assoc || pure_cam)) // Not fully_asso nor cam
  {
    ...
    if(is_tag) {
      if(g_ip->specific_tag) {
        ...
        tagbits = ADDRESS_BITS + EXTRA_TAG_BITS - _log2(capacity_per_die) + _log2(g_ip->tag_assoc * 2 - 1) - _log2(g_ip->nbanks);
      }
      tagbits = (((tagbits + 3) >> 2) << 2);

      num_r_subarray = (int)(capacity_per_die / (g_ip->nbanks * g_ip->block_sz * g_ip->tag_assoc * Ndbl * Nspd) + EPSILON);
      num_c_subarray = (int)((tagbits * g_ip->tag_assoc * Nspd / Ndwl) + EPSILON);
      // burst_length = 1;
    } else {
      num_r_subarray = (int)(capacity_per_die / (g_ip->nbanks * g_ip->block_sz * g_ip->data_assoc * Ndbl * Nspd) + EPSILON);
    ...
    if(pure_cam) {
      ...
      tag_num_r_subarray = (int)(capacity_per_die /
                                 (g_ip->nbanks * tagbits / 8.0 * Ndbl)); // TODO: error check input of tagbits and blocksize //TODO:
    ...
    } else // fully associative
    {
      if(g_ip->specific_tag) {
        ...
        tagbits =
            ADDRESS_BITS + EXTRA_TAG_BITS - _log2(g_ip->block_sz); // TODO: should be the page_offset=log2(page size), but this info
                                                                   // is not avail with CACTI, for McPAT this is no problem.
      }
      tagbits = (((tagbits + 3) >> 2) << 2);

      tag_num_r_subarray = (int)(capacity_per_die / (g_ip->nbanks * g_ip->block_sz * Ndbl));
      tag_num_c_subarray = (int)((tagbits * Nspd / Ndwl) + EPSILON);
      if(tag_num_r_subarray == 0)
        return;
      if(tag_num_r_subarray > MAXSUBARRAYROWS)
        return;
  ...
  if(fully_assoc || pure_cam) {
    ...
    cell.w = g_tp.sram.b_w +
             2 * wire_local.pitch * (g_ip->num_rw_ports - 1 + (g_ip->num_rd_ports - g_ip->num_se_rd_ports) + g_ip->num_wr_ports) +
    ...
    if(is_tag) {
      ...
      cell.w = g_tp.sram.b_w +
               2 * wire_local.pitch * (g_ip->num_rw_ports - 1 + g_ip->num_wr_ports + (g_ip->num_rd_ports - g_ip->num_se_rd_ports)) +
  ...
  if(!(fully_assoc || pure_cam)) {
    ...
    c_b_metal =
        cam_cell.h *
        wire_local.C_per_um; // IBM and SUN design, SRAM array uses dummy cells to fill the blank space due to mismatch on CAM-RAM
    V_b_sense     = (0.05 * g_tp.sram_cell.Vdd > VBITSENSEMIN) ? 0.05 * g_tp.sram_cell.Vdd : VBITSENSEMIN;
    deg_bl_muxing = 1; // FA fix as 1
    // "/ 2.0" below is due to the fact that two adjacent access transistors share drain
    // contacts in a physical layout
    double Cbitrow_drain_cap =
        drain_C_(g_tp.cam.cell_a_w, NCH, 1, 0, cam_cell.w, false, true) / 2.0; // TODO: comment out these two lines
    C_bl                = num_r_subarray * (Cbitrow_drain_cap + c_b_metal);
    dram_refresh_period = 0;
  }
  ...
  if(is_dram && is_main_mem) {
    number_addr_bits_mat =
        MAX((unsigned int)num_addr_b_row_dec, _log2(deg_bl_muxing) + _log2(deg_sa_mux_l1_non_assoc) + _log2(Ndsam_lev_2));
  } else {
