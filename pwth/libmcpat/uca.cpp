...
UCA::UCA(const DynamicParameter &dyn_p)
  ...
  if(!dp.fully_assoc && !dp.pure_cam) {
    ...
    htree_in_add   = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, 0, num_do_b_bank, 0,
                              num_banks_ver_dir, num_banks_hor_dir, Add_htree, true);
    htree_in_data  = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, 0, num_do_b_bank, 0,
                               num_banks_ver_dir, num_banks_hor_dir, Data_in_htree, true);
    htree_out_data = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, 0, num_do_b_bank, 0,
                                num_banks_ver_dir, num_banks_hor_dir, Data_out_htree, true);
  }

  else {

    htree_in_add     = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, num_si_b_bank, num_do_b_bank,
                              num_so_b_bank, num_banks_ver_dir, num_banks_hor_dir, Add_htree, true);
    htree_in_data    = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, num_si_b_bank, num_do_b_bank,
                               num_so_b_bank, num_banks_ver_dir, num_banks_hor_dir, Data_in_htree, true);
    htree_out_data   = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, num_si_b_bank, num_do_b_bank,
                                num_so_b_bank, num_banks_ver_dir, num_banks_hor_dir, Data_out_htree, true);
    htree_in_search  = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, num_si_b_bank, num_do_b_bank,
                                 num_so_b_bank, num_banks_ver_dir, num_banks_hor_dir, Data_in_htree, true);
    htree_out_search = new Htree2(g_ip->wt, bank.area.w, bank.area.h, num_addr_b_bank, num_di_b_bank, num_si_b_bank, num_do_b_bank,
                                  num_so_b_bank, num_banks_ver_dir, num_banks_hor_dir, Data_out_htree, true);
  }
...
double UCA::compute_delays(double inrisetime) {
  ...
  if(dp.is_main_mem) {
    ...
    double cas_latency =
        MAX(delay_array_to_sa_mux_lev_1_decoder, delay_array_to_sa_mux_lev_2_decoder) + delay_from_subarray_out_drv_to_out;
  ...
  if(dp.is_main_mem) {
    ...
    precharge_delay = htree_in_add->delay + bank.htree_in_add->delay + bank.mat.delay_writeback + bank.mat.delay_wl_reset +
...
void UCA::compute_power_energy() {
  ...
  power_routing_to_bank.readOp.leakage +=
      htree_in_add->power.readOp.leakage + htree_in_data->power.readOp.leakage + htree_out_data->power.readOp.leakage;

  power_routing_to_bank.readOp.gate_leakage += htree_in_add->power.readOp.gate_leakage + htree_in_data->power.readOp.gate_leakage +
  ...
  power.writeOp.dynamic = power.readOp.dynamic - bank.mat.power_bitline.readOp.dynamic * dp.num_act_mats_hor_dir +
                          ...
                          power_routing_to_bank.writeOp.dynamic + bank.htree_in_data->power.readOp.dynamic -
  ...
  dyn_read_energy_from_open_page =
      power.readOp.dynamic - (bank.mat.r_predec->power.readOp.dynamic + bank.mat.power_row_decoders.readOp.dynamic +
                              bank.mat.power_bl_precharge_eq_drv.readOp.dynamic + bank.mat.power_sa.readOp.dynamic +
  ...
  dyn_read_energy_remaining_words_in_burst =
      ...
      ((bank.mat.sa_mux_lev_1_predec->power.readOp.dynamic + bank.mat.sa_mux_lev_2_predec->power.readOp.dynamic +
        bank.mat.power_sa_mux_lev_1_decoders.readOp.dynamic + bank.mat.power_sa_mux_lev_2_decoders.readOp.dynamic +
        bank.mat.power_subarray_out_drv.readOp.dynamic) *
           dp.num_act_mats_hor_dir +
       bank.htree_out_data->power.readOp.dynamic + power_routing_to_bank.readOp.dynamic);
  ...
  activate_energy =
      htree_in_add->power.readOp.dynamic + bank.htree_in_add->power_bit.readOp.dynamic * bank.num_addr_b_routed_to_mat_for_act +
  ...
  read_energy = (htree_in_add->power.readOp.dynamic +
                 ...
                 (bank.mat.sa_mux_lev_1_predec->power.readOp.dynamic + bank.mat.sa_mux_lev_2_predec->power.readOp.dynamic +
                  ...
                  bank.mat.power_subarray_out_drv.readOp.dynamic) *
                     dp.num_act_mats_hor_dir +
                 bank.htree_out_data->power.readOp.dynamic + htree_in_data->power.readOp.dynamic) *
                g_ip->burst_len;
  write_energy = (htree_in_add->power.readOp.dynamic +
                 ...
                 g_ip->burst_len;
  precharge_energy =
      (bank.mat.power_bitline.readOp.dynamic + bank.mat.power_bl_precharge_eq_drv.readOp.dynamic) * dp.num_act_mats_hor_dir;
  ...
  leak_power_subbank_closed_page +=
      (bank.mat.r_predec->power.readOp.gate_leakage + bank.mat.b_mux_predec->power.readOp.gate_leakage +
       bank.mat.sa_mux_lev_1_predec->power.readOp.gate_leakage + bank.mat.sa_mux_lev_2_predec->power.readOp.gate_leakage +
       bank.mat.power_row_decoders.readOp.gate_leakage + bank.mat.power_bit_mux_decoders.readOp.gate_leakage +
       bank.mat.power_sa_mux_lev_1_decoders.readOp.gate_leakage + bank.mat.power_sa_mux_lev_2_decoders.readOp.gate_leakage) *
      dp.num_act_mats_hor_dir; //+
  ...
  leak_power_request_and_reply_networks +=
      power_routing_to_bank.readOp.gate_leakage + bank.htree_in_add->power.readOp.gate_leakage +
  ...
  if(dp.is_dram) { // if DRAM, add contribution of power spent in row predecoder drivers, blocks and decoders to refresh power
    refresh_power = (bank.mat.r_predec->power.readOp.dynamic * dp.num_act_mats_hor_dir + bank.mat.row_dec->power.readOp.dynamic) *
  ...
  // TODO: Jung Ho thinks that below is just total hack, which must be avoided.
  /*if (dp.is_main_mem)
  {
