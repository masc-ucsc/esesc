...
Mat::Mat(const DynamicParameter &dyn_p)
  ...
  double R_wire_bit_mux_dec_out =
      num_subarrays_per_row * subarray.num_cols * g_tp.wire_inside_mat.R_per_um * cell.w; // TODO:revisit for FA
  ...
  row_dec = new Decoder(num_dec_signals, false, subarray.C_wl, R_wire_wl_drv_out, false /*is_fa*/, is_dram, true,
  ...
  bit_mux_dec =
      new Decoder(deg_bl_muxing, // This number is 1 for FA or CAM
                  false, C_ld_bit_mux_dec_out, R_wire_bit_mux_dec_out, false /*is_fa*/, is_dram, false, camFlag ? cam_cell : cell);
  sa_mux_lev_1_dec = new Decoder(dp.deg_senseamp_muxing_non_associativity, // This number is 1 for FA or CAM
                                 dp.number_way_select_signals_mat ? true : false, C_ld_sa_mux_lev_1_dec_out, R_wire_sa_mux_dec_out,
                                 false /*is_fa*/, is_dram, false, camFlag ? cam_cell : cell);
  sa_mux_lev_2_dec = new Decoder(dp.Ndsam_lev_2, // This number is 1 for FA or CAM
                                 false, C_ld_sa_mux_lev_2_dec_out, R_wire_sa_mux_dec_out, false /*is_fa*/, is_dram, false,
                                 camFlag ? cam_cell : cell);
  ...
  PredecBlk *r_predec_blk1 =
      new PredecBlk(num_dec_signals, row_dec, C_wire_predec_blk_out, R_wire_predec_blk_out, num_subarrays_per_mat, is_dram, true);
  PredecBlk *r_predec_blk2 =
      new PredecBlk(num_dec_signals, row_dec, C_wire_predec_blk_out, R_wire_predec_blk_out, num_subarrays_per_mat, is_dram, false);
  ...
  PredecBlk *sa_mux_lev_1_predec_blk1 =
      new PredecBlk(dyn_p.deg_senseamp_muxing_non_associativity, sa_mux_lev_1_dec, 0, 0, 1, is_dram, true);
  PredecBlk *sa_mux_lev_1_predec_blk2 =
      new PredecBlk(dyn_p.deg_senseamp_muxing_non_associativity, sa_mux_lev_1_dec, 0, 0, 1, is_dram, false);
  ...
  subarray_out_wire = new Wire(Global, subarray.area.h); // Bug should be subarray.area.w Owen and Sheng
  ...
  { // Although CAM and RAM use different bl pre-charge driver, assuming the precharge p size is the same
    driver_c_gate_load =
        (subarray.num_cols_fa_cam) * gate_C(2 * g_tp.w_pmos_bl_precharge + g_tp.w_pmos_bl_eq, 0, is_dram, false, false);
    ...
    if(!pure_cam) {
      ...
      driver_c_gate_load =
          (subarray.num_cols_fa_ram) * gate_C(2 * g_tp.w_pmos_bl_precharge + g_tp.w_pmos_bl_eq, 0, is_dram, false, false);
  ...
  w_row_predecode_output_wires =
      (branch_effort_predec_blk1_out + branch_effort_predec_blk2_out) * g_tp.wire_inside_mat.pitch * (RWP + ERP + EWP);
  ...
  if(!g_ip->ver_htree_wires_over_array) {
    h_addr_datain_wires =
        (dp.number_addr_bits_mat + dp.number_way_select_signals_mat + (dp.num_di_b_mat + dp.num_do_b_mat) / num_subarrays_per_row) *
  ...
  double area_mat_center_circuitry =
      (r_predec_blk_drv1->area.get_area() + b_mux_predec_blk_drv1->area.get_area() + sa_mux_lev_1_predec_blk_drv1->area.get_area() +
       sa_mux_lev_2_predec_blk_drv1->area.get_area() + way_sel_drv1->area.get_area() + r_predec_blk_drv2->area.get_area() +
       b_mux_predec_blk_drv2->area.get_area() + sa_mux_lev_1_predec_blk_drv2->area.get_area() +
       sa_mux_lev_2_predec_blk_drv2->area.get_area() + r_predec_blk1->area.get_area() + b_mux_predec_blk1->area.get_area() +
       sa_mux_lev_1_predec_blk1->area.get_area() + sa_mux_lev_2_predec_blk1->area.get_area() + r_predec_blk2->area.get_area() +
       b_mux_predec_blk2->area.get_area() + sa_mux_lev_1_predec_blk2->area.get_area() + sa_mux_lev_2_predec_blk2->area.get_area() +
       bit_mux_dec->area.get_area() + sa_mux_lev_1_dec->area.get_area() + sa_mux_lev_2_dec->area.get_area()) *
      (RWP + ERP + EWP);
...
double Mat::compute_delays(double inrisetime) {
  ...
  if(is_fa || pure_cam) {
    ...
    if(is_fa) {
      ...
      C_bl           = subarray.C_bl;
      delay_bl_restore =
          bl_precharge_eq_drv->delay + log((g_tp.sram.Vbitpre - 0.1 * dp.V_b_sense) / (g_tp.sram.Vbitpre - dp.V_b_sense)) *
...
double Mat::compute_bit_mux_sa_precharge_sa_mux_wr_drv_wr_mux_h() {
  ...
  double height =
      compute_tr_width_after_folding(g_tp.w_pmos_bl_precharge, camFlag ? cam_cell.w : cell.w / (2 * (RWP + ERP + SCHP))) +
  ...
  if(dp.Ndsam_lev_2 > 1) {
    height += compute_tr_width_after_folding(g_tp.w_nmos_sa_mux,
                                             cell.w * deg_bl_muxing * dp.Ndsam_lev_1 / (RWP + ERP)); // sense_amp_mux_height
    // height_senseamp_mux_decode_output_wires =  Ndsam * wire_inside_mat_pitch * (RWP + ERP);
  ...
  // TODO: Jung Ho thinks this should be uncommented...
...
double Mat::compute_cam_delay(double inrisetime) {
  ...
  double /*Wdecdrivep, Wdecdriven, Wfadriven, Wfadrivep, Wfadrive2n, Wfadrive2p, Wfadecdrive1n, Wfadecdrive1p,
    Wfadecdrive2n, Wfadecdrive2p, Wfadecdriven, Wfadecdrivep, Wfaprechn,
     Wfainvn, Wfainvp,   Wdecnandn, Wdecnandp,*/
      Wdummyinvn,
      Wdummyinvp, Wfanorn, Wfanorp, Waddrnandn, Waddrnandp, W_hit_miss_n, W_hit_miss_p, Wfaprechp, Wdummyn;
  ...
  double R_bl_precharge =
      tr_R_on(g_tp.w_pmos_bl_precharge, PCH, 1, is_dram, false, false); // Assuming CAM and SRAM have same Pre_eq_dr
  ...
  out_time_ramp = sl_data_drv->compute_delay(
      inrisetime); // After entering one mat, start to consider the inrisetime from 0(0 is passed from outside)
  ...
  c_intrinsic =
      drain_C_(Waddrnandn, NCH, 2, 1, g_tp.cell_h_def, is_dram) + drain_C_(Waddrnandp, PCH, 1, 1, g_tp.cell_h_def, is_dram) * 2;
  ...
  dynSearchEng +=
      (c_intrinsic * (subarray.num_rows + 1) + c_gate_load * 2) * g_tp.peri_global.Vdd * g_tp.peri_global.Vdd; //  * Ntbl;
  ...
  c_intrinsic =
      drain_C_(Wdummyinvn, NCH, 1, 1, g_tp.cell_h_def, is_dram) + drain_C_(Wdummyinvp, NCH, 1, 1, g_tp.cell_h_def, is_dram);
  ...
  double Iport            = cmos_Isub_leakage(g_tp.cam.cell_a_w, 0, 1, nmos); // TODO: how much is the idle time? just by *2?
  double Iport_erp        = cmos_Isub_leakage(g_tp.cam.cell_a_w, 0, 2, nmos);
  double Icell            = cmos_Isub_leakage(g_tp.cam.cell_nmos_w, g_tp.cam.cell_pmos_w, 1, inv) * 2;
  double Icell_comparator = cmos_Isub_leakage(Wdummyn, Wdummyn, 1, inv) * 2; // approx XOR with Inv
  ...
  power_matchline.searchOp.leakage += leak_power_cc_inverters_sram_cell + leak_comparator_cam_cell +
                                      leak_power_acc_tr_RW_or_WR_port_sram_cell +
                                      leak_power_acc_tr_RW_or_WR_port_sram_cell * (RWP + EWP - 1) +
                                      leak_power_RD_port_sram_cell * ERP + leak_power_SCHP_port_sram_cell * SCHP;
  ...
  power_matchline.searchOp.leakage +=
      (subarray.num_rows + 1) * cmos_Isub_leakage(Waddrnandn, Waddrnandp, 2, nand) * g_tp.cam_cell.Vdd;
  power_matchline.searchOp.leakage += (subarray.num_rows + 1) * cmos_Isub_leakage(Wfanorn, Wfanorp, 2, nor) * g_tp.cam_cell.Vdd;
  // In idle states, the hit/miss txs are open therefore no Isub
  ...
  double Ig_port_erp        = cmos_Ig_leakage(g_tp.cam.cell_a_w, 0, 1, nmos);
  double Ig_cell            = cmos_Ig_leakage(g_tp.cam.cell_nmos_w, g_tp.cam.cell_pmos_w, 1, inv) * 2;
  double Ig_cell_comparator = cmos_Ig_leakage(Wdummyn, Wdummyn, 1, inv) * 2; // cmos_Ig_leakage(Wdummyn, 0, 2, nmos)*2;
  ...
  power_matchline.searchOp.gate_leakage +=
      (subarray.num_rows + 1) * cmos_Ig_leakage(Waddrnandn, Waddrnandp, 2, nand) * g_tp.cam_cell.Vdd;
...
double Mat::compute_bitline_delay(double inrisetime) {
  ...
  if(is_dram == true) {
    ...
    double Ig_cell     = cmos_Ig_leakage(g_tp.sram.cell_nmos_w, g_tp.sram.cell_pmos_w, 1, inv);
  ...
  double C_drain_bit_mux =
      drain_C_(g_tp.w_nmos_b_mux, NCH, 1, 0, camFlag ? cam_cell.w : cell.w / (2 * (RWP + ERP + SCHP)), is_dram);
  double R_bit_mux = tr_R_on(g_tp.w_nmos_b_mux, NCH, 1, is_dram);
  double C_drain_sense_amp_iso =
  ...
  if(is_dram) {
    ...
    if(deg_bl_muxing > 1) {
      ...
      dynRdEnergy +=
          (C_bl + 2 * C_drain_bit_mux) * 2 * dp.V_b_sense * g_tp.sram_cell.Vdd * subarray.num_cols /* num_subarrays_per_mat*/;
      dynRdEnergy += (2 * C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) * 2 * dp.V_b_sense *
                     g_tp.sram_cell.Vdd * (1.0 /*subarray.num_cols * num_subarrays_per_mat*/ / deg_bl_muxing);
      ...
      tau = (R_cell_pull_down + R_cell_acc) * (C_bl + C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) +
            R_bl * C_bl / 2 + R_sense_amp_iso * (C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux);
      ...
      dynWriteEnergy += (((1.0 /*subarray.num_cols * num_subarrays_per_mat*/ / deg_bl_muxing) / deg_senseamp_muxing) *
                        ...
                        g_tp.sram_cell.Vdd * g_tp.sram_cell.Vdd;
    }
  ...
  double outrisetime = 0; // TODO: Jung Ho thinks this one must be revisited.
  return outrisetime;
}
...
double Mat::compute_subarray_out_drv(double inrisetime) {
  ...
  power_subarray_out_drv.readOp.leakage +=
      cmos_Isub_leakage(g_tp.min_w_nmos_, p_to_n_sz_r * g_tp.min_w_nmos_, 1, inv, is_dram) * g_tp.peri_global.Vdd;
  power_subarray_out_drv.readOp.gate_leakage +=
      cmos_Ig_leakage(g_tp.min_w_nmos_, p_to_n_sz_r * g_tp.min_w_nmos_, 1, inv) * g_tp.peri_global.Vdd;
  ...
  C_ld = drain_C_(g_tp.min_w_nmos_, NCH, 1, 1, g_tp.cell_h_def, is_dram) +
         ...
         drain_C_(g_tp.w_nmos_sa_mux, NCH, 1, 0,
                  camFlag ? cam_cell.w : cell.w * deg_bl_muxing * dp.Ndsam_lev_1 / (RWP + ERP + SCHP), is_dram);
  tf         = rd * C_ld;
  this_delay = horowitz(inrisetime, tf, 0.5, 0.5, RISE);
  delay_subarray_out_drv += this_delay;
  inrisetime = this_delay / (1.0 - 0.5);
  power_subarray_out_drv.readOp.dynamic += C_ld * 0.5 * g_tp.peri_global.Vdd * g_tp.peri_global.Vdd;
  power_subarray_out_drv.readOp.leakage +=
      cmos_Isub_leakage(g_tp.min_w_nmos_, p_to_n_sz_r * g_tp.min_w_nmos_, 1, inv) * g_tp.peri_global.Vdd;
  power_subarray_out_drv.readOp.gate_leakage +=
      cmos_Ig_leakage(g_tp.min_w_nmos_, p_to_n_sz_r * g_tp.min_w_nmos_, 1, inv) * g_tp.peri_global.Vdd;

  // delay of signal through pass-transistor to input of subarray output driver.
  rd   = tr_R_on(g_tp.w_nmos_sa_mux, NCH, 1, is_dram);
  C_ld = dp.Ndsam_lev_2 * drain_C_(g_tp.w_nmos_sa_mux, NCH, 1, 0,
                                   camFlag ? cam_cell.w : cell.w * deg_bl_muxing * dp.Ndsam_lev_1 / (RWP + ERP + SCHP), is_dram) +
...
double Mat::compute_comparator_delay(double inrisetime) {
  ...
  gatelkgCurrent +=
      cmos_Ig_leakage(g_tp.w_comp_n, g_tp.w_comp_n, 1, inv, is_dram) * 4 * A; // for gate leakage this equals to a inverter
...
void Mat::compute_power_energy() {
  ...
  if(!(is_fa || pure_cam)) {
    ...
    power.readOp.dynamic += power_bl_precharge_eq_drv.readOp.dynamic + power_sa.readOp.dynamic + power_bitline.readOp.dynamic +
                            power_subarray_out_drv.readOp.dynamic;
    ...
    power_bl_precharge_eq_drv.readOp.dynamic =
        bl_precharge_eq_drv->power.readOp.dynamic + cam_bl_precharge_eq_drv->power.readOp.dynamic;
  ...
  if(!(is_fa || pure_cam)) {
    ...
    power.readOp.leakage += power_bitline.readOp.leakage + power_bl_precharge_eq_drv.readOp.leakage + power_sa.readOp.leakage +
                            power_subarray_out_drv.readOp.leakage;
    ...
    power.readOp.gate_leakage += power_bitline.readOp.gate_leakage + power_bl_precharge_eq_drv.readOp.gate_leakage +
                                 power_sa.readOp.gate_leakage + power_subarray_out_drv.readOp.gate_leakage;
    // cout<<"leakage"<<power.readOp.leakage<<endl;
