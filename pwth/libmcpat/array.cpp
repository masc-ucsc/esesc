...
void ArrayST::optimize_array_peq()
{
	...
	if(l_ip.is_cache && assoc!= 0.0) {// It is a cache
		...
		local_result.power.readOp.dynamic = 
			cache.EvalEquation("dynamic");
	...
	{
		double tech = l_ip.F_sz_um;
		double ports = l_ip.num_rw_ports + l_ip.num_rd_ports
			+ l_ip.num_wr_ports + l_ip.num_search_ports;
