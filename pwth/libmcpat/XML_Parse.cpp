...
void ParseXML::updateCntrValues(vector<uint32_t> *stats_vector, map<string,int> mcpat_map)
{
  ...
  for (uint32_t ii = 0; ii < sys.number_of_cores; ii++){
    ...
    if ( check_cntr(str) ) sys.core[i].total_instructions= cntr_pos_value(str);
    sprintf(str,"P(%u)_core_int_instructions",i);
    if ( check_cntr(str) ) sys.core[i].int_instructions = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_fp_instructions",i);
    if ( check_cntr(str) ) sys.core[i].fp_instructions=cntr_pos_value(str);
    sprintf(str,"P(%u)_core_branch_instructions",i);
    if ( check_cntr(str) ) sys.core[i].branch_instructions=cntr_pos_value(str);
    sprintf(str,"P(%u)_core_branch_mispredictions",i);
    if ( check_cntr(str) ) sys.core[i].branch_mispredictions = cntr_pos_value(str);
    ...
    if ( check_cntr(str) ) sys.core[i].int_regfile_reads=cntr_pos_value(str);
    sprintf(str,"P(%u)_core_float_regfile_reads",i);
    if ( check_cntr(str) ) sys.core[i].float_regfile_reads = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_int_regfile_writes",i);
    if ( check_cntr(str) ) sys.core[i].int_regfile_writes =cntr_pos_value(str);
    sprintf(str,"P(%u)_core_float_regfile_writes",i);
    if ( check_cntr(str) ) sys.core[i].float_regfile_writes =cntr_pos_value(str);
    ...
    if ( check_cntr(str) ) sys.core[i].icache.read_misses=cntr_pos_value(str);
#if 0
    ...
    if ( check_cntr(str) ) tmp += cntr_pos_value(str);
    if ( check_cntr(str) ) sys.core[i].dtlb.total_accesses=tmp;
    sprintf(str,"PTLB(%u)_read_misses",i);
    if ( check_cntr(str) ) sys.core[i].dtlb.total_misses= cntr_pos_value(str);
#else
    sprintf(str,"DL1(%u)_read_accesses",i);
    if ( check_cntr(str) ) sys.core[i].dtlb.total_accesses=sys.core[i].dcache.read_accesses;
#endif
    ...
    if ( check_cntr(str) ) sys.core[i].LoadQ.write_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_loadq_read_accesses",i);
    if ( check_cntr(str) ) sys.core[i].LoadQ.read_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_storq_read_accesses",i);
    if ( check_cntr(str) ) sys.core[i].LSQ.read_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_storq_write_accesses",i);
    if ( check_cntr(str) ) sys.core[i].LSQ.write_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_ssit_read_accesses",i);
    if ( check_cntr(str) ) sys.core[i].ssit.read_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_ssit_write_accesses",i);
    if ( check_cntr(str) ) sys.core[i].ssit.write_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_lfst_read_accesses",i);
    if ( check_cntr(str) ) sys.core[i].lfst.read_accesses = cntr_pos_value(str);
    sprintf(str,"P(%u)_core_lfst_write_accesses",i);
    if ( check_cntr(str) ) sys.core[i].lfst.write_accesses = cntr_pos_value(str);
    if (sys.core[i].scoore)
    {
      sprintf(str,"P(%u)_core_strb_read_accesses",i);
      if ( check_cntr(str) ) sys.core[i].strb.read_accesses = cntr_pos_value(str);
      sprintf(str,"P(%u)_core_strb_write_accesses",i);
      if ( check_cntr(str) ) sys.core[i].strb.write_accesses = cntr_pos_value(str);
      sprintf(str,"P(%u)_core_ldrb_read_accesses",i);
      if ( check_cntr(str) ) sys.core[i].ldrb.read_accesses = cntr_pos_value(str);
      sprintf(str,"P(%u)_core_ldrb_write_accesses",i);
      if ( check_cntr(str) ) sys.core[i].ldrb.write_accesses = cntr_pos_value(str);
    }
  }
  ...
  if(sys.number_of_GPU) {
    ...
    for (int ii = 0; ii < sys.gpu.number_of_SMs; ii++){
      ...
      for (int j = 0; j < sys.gpu.homoSM.number_of_lanes; j++){
        ...
        sprintf(str,"P(%u)_core_int_instructions",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].int_instructions = cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_fp_instructions",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].fp_instructions=cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_branch_instructions",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].branch_instructions=cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_total_cycles",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].total_cycles= cntr_pos_value(str);
        sprintf(str,"P(%u)_core_int_regfile_reads",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].int_regfile_reads=cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_int_regfile_writes",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].int_regfile_writes =cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_function_calls",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].function_calls=cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_bypassbus_access",i);
        if ( check_cntr(str) ) sys.gpu.SMS[ii].lanes[j].bypassbus_access= cntr_pos_value(str)/nl;
        sprintf(str,"P(%u)_core_storq_read_accesses",i);
...
*/
      ...
      if ( check_cntr(str) ) sys.gpu.SMS[ii].dcache.write_accesses = cntr_pos_value(str);
      sprintf(str,"DL1G(%u)_read_misses",i);
      if ( check_cntr(str) ) sys.gpu.SMS[ii].dcache.read_misses= cntr_pos_value(str);
    ...
    if ( check_cntr(str) ) sys.gpu.L2.read_accesses=cntr_pos_value(str);
    sprintf(str,"L2G(%u)_write_accesses",i);
    if ( check_cntr(str) ) sys.gpu.L2.write_accesses=cntr_pos_value(str);
    sprintf(str,"L2G(%u)_read_misses",i);
    if ( check_cntr(str) ) sys.gpu.L2.read_misses=cntr_pos_value(str);
  ...
  for (uint32_t  i = 0; i < sys.number_of_L2s; i++){
    ...
    if ( check_cntr(str) ) sys.L2[i].read_accesses=cntr_pos_value(str);
    sprintf(str,"L2(%u)_write_accesses",i);
    if ( check_cntr(str) ) sys.L2[i].write_accesses=cntr_pos_value(str);
    ...
    if (sys.core[0].scoore) //FIXME: only homo cores
    {
      if(sys.core[i].VPCfilter.dcache_config[0]>0){
        ...
        if ( check_cntr(str) ) sys.core[i].VPCfilter.read_accesses = cntr_pos_value(str);
        sprintf(str,"FL2(%u)_write_accesses",i);
        if ( check_cntr(str) ) sys.core[i].VPCfilter.write_accesses = cntr_pos_value(str);
  ...
  for (uint32_t  i = 0; i < sys.number_of_STLBs; i++){
    ...
    sprintf(str,"STLB(%u)_read_misses",i);
    if ( check_cntr(str) ) sys.STLB[i].read_misses= cntr_pos_value(str);
    sprintf(str,"STLB(%u)_write_accesses",i);
    if ( check_cntr(str) ) sys.STLB[i].write_accesses= cntr_pos_value(str);
  }

  for (uint32_t  i = 0; i < sys.number_of_L3s; i++){
    sprintf(str,"L3(%u)_read_accesses",i);
    if ( check_cntr(str) ) sys.L3[i].read_accesses=cntr_pos_value(str);
    sprintf(str,"L3(%u)_write_accesses",i);
    if ( check_cntr(str) ) sys.L3[i].write_accesses=cntr_pos_value(str);
    sprintf(str,"L3(%u)_read_misses",i);
    if ( check_cntr(str) ) sys.L3[i].read_misses=cntr_pos_value(str);
    sprintf(str,"L3(%u)_write_misses",i);
    if ( check_cntr(str) ) sys.L3[i].write_misses=cntr_pos_value(str);
  }
  ...
  for (uint32_t  i = 0; i < sys.number_of_NoCs; i++){
    ...
    if ( check_cntr(str) ) writes=cntr_pos_value(str);
