    ...
    for(int j = 2; j < 5; j++) {
      for(int k = 0; k < ROUTER_TYPES; k++) {
        for(int l = 0; l < 7; l++) {
          ...
          assert(fscanf(cont, "%[^\n]\n", line) != EOF);
          sscanf(line, "%[^:]: %d %d %d %d %d %d %d %d", jk, &temp[0], &temp[1], &temp[2], &temp[3], &temp[4], &temp[5], &temp[6],
                 &temp[7]);
        }
      }
    }
...
void Nuca::sim_nuca() {
  ...
  if(g_ip->nuca_bank_count != 0) { // simulate just one bank
    if(g_ip->nuca_bank_count != 2 && g_ip->nuca_bank_count != 4 && g_ip->nuca_bank_count != 8 && g_ip->nuca_bank_count != 16 &&
    ...
    for(wr = wt_min; wr <= wt_max; wr++) {
      ...
      for(ro = 0; ro < ROUTER_TYPES; ro++) {
        ...
        if(it < 7) {
          ...
          nuca_list.back()->nuca_pda.delay = opt_acclat + cont_stats[l2_c][core_in][ro][6][num_cyc / 2 - 1];
          nuca_list.back()->contention     = cont_stats[l2_c][core_in][ro][6][num_cyc / 2 - 1];
        }
...
void Nuca::print_nuca(nuca_org_t *fr) {
  ...
  printf("Cache dimension (mm x mm) - %g x %g\n", fr->nuca_pda.area.h, fr->nuca_pda.area.w);
  ...
  printf("\tHorizontal link energy -dynamic/access %g (nJ)\n"
         "\t                       -leakage %g (nW)\n\n",
         fr->h_wire->power.readOp.dynamic * 1e9, fr->h_wire->power.readOp.leakage * 1e9);
  printf("\tVertical link energy -dynamic/access %g (nJ)\n"
         "\t                     -leakage %g (nW)\n\n",
         fr->v_wire->power.readOp.dynamic * 1e9, fr->v_wire->power.readOp.leakage * 1e9);
  printf("\n\n");
...
nuca_org_t *Nuca::find_optimal_nuca(list<nuca_org_t *> *n, min_values_t *minval) {
  ...
  for(niter = n->begin(); niter != n->end(); niter++) {
    ...
    printf("NUCA___stats %d \tbankcount: lat = %g \tdynP = %g \twt = %d\t "
           ...
           (*niter)->bank_count, (*niter)->nuca_pda.delay, (*niter)->nuca_pda.power.readOp.dynamic, (*niter)->h_wire->wt,
           (*niter)->bank_pda.power.readOp.dynamic, (*niter)->nuca_pda.power.readOp.leakage, (*niter)->nuca_pda.cycle_time);
    ...
    if(g_ip->ed == 1) {
      ...
      if(v) {
        cost = (d * ((*niter)->nuca_pda.delay / minval->min_delay) + c * ((*niter)->nuca_pda.cycle_time / minval->min_cyc) +
...
void Nuca::calculate_nuca_area(nuca_org_t *nuca) {
  nuca->nuca_pda.area.h =
      nuca->rows * ((nuca->h_wire->wire_width + nuca->h_wire->wire_spacing) * nuca->router->flit_size + nuca->bank_pda.area.h);

  nuca->nuca_pda.area.w =
      nuca->columns * ((nuca->v_wire->wire_width + nuca->v_wire->wire_spacing) * nuca->router->flit_size + nuca->bank_pda.area.w);
}
