...
double cmos_Isub_leakage(double nWidth, double pWidth, int fanin, enum Gate_type g_type, bool _is_dram, bool _is_cell,
    ...
    if(fanin == 1) {
      ...
      if(topo == parallel) {
        Isub = nmos_leak * fanin /
               num_states; // only when all tx are off, leakage power is non-zero. The possibility of this state is 1/num_states
      } else {
        for(num_off_tx = 1; num_off_tx <= fanin; num_off_tx++) // when num_off_tx ==0 there is no leakage power
        {
          // Isub += nmos_leak*pow(UNI_LEAK_STACK_FACTOR,(num_off_tx-1))*(factorial(fanin)/(factorial(fanin,
  ...
  case pmos:
    if(fanin == 1) {
      ...
      if(topo == parallel) {
        ...
        for(num_off_tx = 1; num_off_tx <= fanin; num_off_tx++) // when num_off_tx ==0 there is no leakage power
        {
          // Isub += pmos_leak*pow(UNI_LEAK_STACK_FACTOR,(num_off_tx-1))*(factorial(fanin)/(factorial(fanin,
          // num_off_tx)*factorial(num_off_tx)));
...
double cmos_Ig_leakage(double nWidth, double pWidth, int fanin, enum Gate_type g_type, bool _is_dram, bool _is_cell, bool _is_wl_tr,
