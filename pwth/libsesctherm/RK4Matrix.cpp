...
void sescthermRK4Matrix::free_mem()
{
  ...
  if(k1) { free(k1); k1 = 0x0; }
  if(k2) { free(k2); k2 = 0x0; }
  if(k3) { free(k3); k3 = 0x0; }
  if(k4) { free(k4); k4 = 0x0; }
  if(t1) { free(t1); t1 = 0x0; }
  if(t2) { free(t2); t2 = 0x0; }
  if(ytemp) { free(ytemp); ytemp = 0x0; }
...
void sescthermRK4Matrix::solve_matrix(ConfigData *conf, SUElement_t *temp_vector, 
    MATRIX_DATA timestep, std::vector<ModelUnit *>& matrix_model_units)
{
  ...
  for (t = 0, new_h = MIN_STEP; t + new_h <= timestep; t+= h* num_iter) {
    // printf("t %g new_h %g\n", t, new_h);
    h = new_h;
    new_h = RK4(unsolved_matrix_dyn_, _temporary_temp_vector, power_vector, _numelems, 
        h, _temporary_temp_vector, num_iter);
  }
  /* remainder    */
  if (timestep > t) {
    new_h = RK4(unsolved_matrix_dyn_, _temporary_temp_vector, power_vector, _numelems, 
        timestep - t, _temporary_temp_vector,1);
  }
