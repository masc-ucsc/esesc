...
void sescthermRK4Matrix::matrixVectMult(MATRIX_DATA *vout, MATRIX_DATA **m,
    MATRIX_DATA *vin, int32_t n)
{
 ...
 */
void sescthermRK4Matrix::RK4_core(MATRIX_DATA **c, MATRIX_DATA *y, 
    MATRIX_DATA *k1, MATRIX_DATA *pow, int32_t n, MATRIX_DATA h, MATRIX_DATA *yout)
{
  if(!k2) {
		k2 = dvector(n);
		k3 = dvector(n);
...
MATRIX_DATA sescthermRK4Matrix::RK4(MATRIX_DATA **c, MATRIX_DATA *y, 
    MATRIX_DATA *power, int32_t n, 
    MATRIX_DATA h, MATRIX_DATA *yout, int32_t num_iter)
{
  int32_t i;
  MATRIX_DATA max = 0.0, new_h = h;

  if(!k1) k1 = dvector(n);
  if(!t1) t1 = dvector(_numRowsInCoeffs);
  if(!t2) t2 = dvector(_numRowsInCoeffs);
  if(!ytemp) ytemp = dvector(_numRowsInCoeffs);
