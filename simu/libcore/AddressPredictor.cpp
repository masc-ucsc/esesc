...
AddrType StrideAddressPredictor::predict(AddrType ppc, int distance, bool inLines) {
  ...
  bool ld_tracking = ppc == 0x10006540
       || ppc == 0x10006544;
  if(ld_tracking)
    printf("STRIDE ldpc:%x dist:%d addr:%llx delta:%d conf:%d\n",ppc, distance, bimodal.get_addr(ppc), delta, bimodal.get_conf(ppc));
  ...
  ,nhist(_nhist)
{
  ...
  for (uint64_t i = 2; i <= nhist; i++) {
    m[i] = (int) (((double) VTAGE_MINHIST
          * pow ((double) (VTAGE_MAXHIST) / (double) VTAGE_MINHIST, (double) (i - 1) / (double) ((nhist - 1)))) + 0.5);
  }
...
AddrType vtage::predict(AddrType pc, int distance, bool inLines) {
  ...
  for(int i=0;i<distance;i++) {
    ...
    if (pc == 0x12000e220)
      printf("dist:%d addr:%llx pred_delta:%d delta:%d conf:%d\n", i, addr, pred_delta, (int)addr-(int)last[get_last_index(pc)].addr, pred_conf);
#endif
  ...
  if (!pa.predictable && last_addr != addr && last_addr) {
    if (pa.source_pc) {
      ...
      if (pred_addr == addr) {
        if (pa.conf<63)
          pa.conf++;
        //PTRACE("pc:%llx predictable with addr:%llx pc:%llx c:%d factor:%d base:%llx",pc,addr,pa.source_pc,pa.conf,adhist[pa.source_pc].factor, adhist[pa.source_pc].base);
      }else{
        if (pa.conf>0)
          pa.conf--;
      ...
      for(size_t i=0;i<last_pcs.size();i++) {
        ...
        for(int factor=1;factor<32;factor++) { // Try some easy to multiply factors (Power2)
          ...
          if (pred_addr == addr) {
            ...
            //PTRACE("pc:%llx matches addr:%llx pc:%llx with la:%llx + factor:%d * d:%d, base:%llx",pc,addr,ppc,last_addr,factor,adhist[ppc].data_delta,base);
            pa.source_pc = ppc;
            pa.conf      = 1;
#if 0
            pa.base      = base;
...
void IndirectAddressPredictor::performed(MemObj *DL1, AddrType pc, AddrType ld1_addr) {
  ...
  //MSG("trying %s pc:%llx cl:%llx b:%llx f:%d d:%d",DL1->getName(), (long long)pc,(long long)pred_addr,(long long)adhist[pc].base, adhist[pc].factor, ld1_data);
