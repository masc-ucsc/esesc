...
sub powerStats {
  ...
  printf "********************************************************************************************************\n";
  print "                Proc      RF          ROB        fetch        EXE         RNU         LSU         Membus\n";


  for(my $i=0;$i<$nCPUs;$i++) {
      next unless( $cf->getResultField("P(${i})","clockTicks") );
      
      printf "Dynamic Power",$i;
      printf " %3d ",$i;
      ...
      printf "Leakage Power",$i;
      printf " %3d ",$i;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_RF","v");
      printf " %10.0f " ,$mypow;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_ROB","v");
      printf " %10.0f " ,$mypow;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_fetch","v");
      printf " %10.0f " ,$mypow;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_EXE","v");
      printf " %10.0f " ,$mypow;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_RNU","v");
      printf " %10.0f " ,$mypow;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_LSU","v");
      printf " %10.0f " ,$mypow;

      my $mypow = $cf->getResultField("pwrLkgP(${i})_MemBus","v");
      printf " %10.0f " ,$mypow;
