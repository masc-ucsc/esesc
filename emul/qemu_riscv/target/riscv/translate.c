        ...
        int rs2)
{
    ...
    CASE_OP_32_64(OPC_RISC_MUL):
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_MULT, rs1, rs2, rd);
        if (!has_ext(ctx, RVM)) {
            goto do_illegal;
        }
...
#endif
    case OPC_RISC_DIV:
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_DIV, rs1, rs2, rd);
        if (!has_ext(ctx, RVM)) {
            goto do_illegal;
        }
...
static void gen_jal(DisasContext *ctx, int rd, target_ulong imm)
{
    ...
    if (rd != 0) {
      ...
      ESESC_TRACE_LCTRL(ctx->base.pc_next,ctx->base.pc_next + imm,iBALU_LJUMP, 0, 0, LREG_InvalidOutput);
    }
                     ...
                     target_long imm)
{
    ...
    case OPC_RISC_JALR:
        ...
        if (rd != 0) {
            if (rd==1)
              ESESC_TRACE_LCTRL2(ctx->base.pc_next,cpu_pc,iBALU_RCALL, rs1, 0, rd);
            else
              ESESC_TRACE_LCTRL2(ctx->base.pc_next,cpu_pc,iBALU_RJUMP, rs1, 0, rd);
            tcg_gen_movi_tl(cpu_gpr[rd], ctx->pc_succ_insn);
        }else{
            if(rs1==1)
              ESESC_TRACE_LCTRL2(ctx->base.pc_next,cpu_pc,iBALU_RET, rs1, 0, LREG_InvalidOutput);
            else
              ESESC_TRACE_LCTRL2(ctx->base.pc_next,cpu_pc,iBALU_RJUMP, rs1, 0, LREG_InvalidOutput);
        }
        tcg_gen_exit_tb(0);
                       ...
                       target_long bimm)
{
...

    {
      ...
      gen_get_gpr(data1, rs1);
      gen_get_gpr(data2, rs2);
      ESESC_TRACE_LBRANCH(ctx->base.pc_next, 0, data1, data2, rs1, rs2, LREG_InvalidOutput); // Not Taken path
      tcg_temp_free(data1);
      tcg_temp_free(data2);
    }
...

    {
      ...
      gen_get_gpr(data1, rs1);
      gen_get_gpr(data2, rs2);
      ESESC_TRACE_LBRANCH(ctx->base.pc_next, ctx->base.pc_next + bimm, data1, data2, rs1, rs2, LREG_InvalidOutput); // Taken path
      tcg_temp_free(data1);
      tcg_temp_free(data2);
    }
                      ...
                      int rd, int rs1, int rs2)
{
    ...
    case OPC_RISC_LR:
        ...
        ESESC_TRACE_MEM(ctx->base.pc_next,src1,iSALU_LL, rs1, rs2, rd);
        if (rl) {
    ...
    case OPC_RISC_SC:
        ...
        gen_get_gpr(src1, rs1);
        ESESC_TRACE_MEM(ctx->base.pc_next,src1,iSALU_SC, rs1, rs2, rd); // Atomic. Maybe new encode?
                         ...
                         int rs1, int rs2, int rm)
{
    ...
    case OPC_RISC_FADD_S:
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_FPALU, LREG_FP0+rs1, LREG_FP0+rs2, LREG_FP0+rd);
        if (!has_ext(ctx, RVF)) {
            goto do_illegal;
        }
        gen_set_rm(ctx, rm);
    ...
    case OPC_RISC_FMUL_S:
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_FPMULT, LREG_FP0+rs1, LREG_FP0+rs2, LREG_FP0+rd);
        if (!has_ext(ctx, RVF)) {
            goto do_illegal;
        }
        gen_set_rm(ctx, rm);
    ...
    case OPC_RISC_FDIV_S:
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_FPDIV, LREG_FP0+rs1, LREG_FP0+rs2, LREG_FP0+rd);
        if (!has_ext(ctx, RVF)) {
            goto do_illegal;
        }
        gen_set_rm(ctx, rm);
    ...
    case OPC_RISC_FCVT_W_S:
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_FPALU, LREG_FP0+rs1, 0, rd);
    ...
    case OPC_RISC_FCVT_S_W:
        ESESC_TRACE_ALU(ctx->base.pc_next, iCALU_FPALU, rs1, 0, LREG_FP0+rd);
                       ...
                       int csr)
{
    ...
    ESESC_TRACE_ALU(ctx->base.pc_next, iRALU, 0, 0, LREG_InvalidOutput); // Syscall signature (missing deps, TO FIX)
...
static void decode_RV32_64C1(DisasContext *ctx)
{
    ...
    case 3:
        if (rd_rs1 == 2) {
            ...
            ESESC_TRACE_ALU(ctx->base.pc_next, iAALU, 0, 0, rd_rs1);
...
static void decode_RV32_64G(DisasContext *ctx)
{
    ...
    case OPC_RISC_LUI:
        ...
        ESESC_TRACE_ALU(ctx->base.pc_next, iAALU, 0, 0, rd);
