...
// Constructor {{{1
{
  ...
  for(sect = bfdFile->sections; sect; sect = sect->next) {
    ...
    if(!strcmp(bfd_get_section_name(bfdFile, sect), ".bss") ||
       !strcmp(bfd_get_section_name(bfdFile, sect), "__libc_freeres_ptrs")) {
  ...
  for(sect = bfdFile->sections; sect; sect = sect->next) {
    if((bfd_get_section_flags(bfdFile, sect) & SEC_ALLOC) && (bfd_get_section_flags(bfdFile, sect) & SEC_LOAD) &&
       bfd_section_vma(bfdFile, sect) && bfd_section_size(bfdFile, sect)) {
  ...
  memcpy((text_buffer + dst_offset), (data_buffer + src_offset), (text_end - last_text_section_end));
  // padding 3:
...
uint32_t ProgramStandAlone::do_syscall(ProgramBase *prog, int num, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4,
