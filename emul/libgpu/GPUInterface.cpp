...
extern "C" uint32_t GPUReader_setNumThreads(uint64_t binnumthreads, uint64_t binblockthreads) {
  ...
  IS(MSG("Numthreads set to %llu blockSize set to %llu, maxthreadstobesampled is %lld", numThreads, blockSize, maxSamplingThreads));
  return 0;
}
...
extern "C" void GPUReader_mapcudaMemcpy(uint32_t addr0, uint32_t addr1, uint32_t size, uint32_t kind, void *env,
                                        uint32_t *cpufid) { /*{{{*/
  uint32_t      dev_addr  = 0;
  ...
  if(unifiedCPUGPUmem) {
    ...
    // op = 4  (alu+br)    fcf4: f5d1f07c        pld     [r1, #124];       0x7c │~ op = 1  (8ld+9alu)  fcf8: e8b151f8        ldm
    // r1!, {r3, r4, r5, r6, r7, r8, ip, lr}                                                        │~ op = 4  (2alu)      fcfc:
    // e2522020        subs    r2, r2, #32                                                                                  │~ op = 2
    // (8st+9alu)  fd00: e8a051f8        stmia   r0!, {r3, r4, r5, r6, r7, r8, ip, lr} │~ op =3  (1br)       fd04: aafffffa bge fcf4
    // <memcpy+0x44>
...
extern "C" void GPUReader_setCurrentKernel(const char *local_kernel_name) { /*{{{*/
  CUDAKernelMapType::iteratoriter  = kernels.begin();
  uint32_t      size  = 0;
  bool          found = false;
...
extern "C" void GPUReader_getKernelParameters(uint32_t *DFL_REG32, uint32_t *DFL_REG64, uint32_t *DFL_REGFP, uint32_t *DFL_REGFP64,
                                              uint32_t *SM_REG32, uint32_t *SM_REG64, uint32_t *SM_REGFP, uint32_t *SM_REGFP64,
...
#else
extern "C" uint64_t GPUReader_encode_rawaddr(AddrType addr_ptr, uint32_t blockid, uint32_t warpid, uint32_t pe_id,
                                             bool sharedAddr) { /*{{{*/
