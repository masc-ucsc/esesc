                  ...
                  if (kernel->isBarrierBlock(trace_currentbbid)){
                    ...
                    if (!(kernel->isDummyBB(trace_currentbbid))){
                      ...
                      if (istsfifoBlocked[glofid]) {
                        ...
                        //MSG("\nEMULATOR: Inserting Instruction PC=%x SM[%d] PE [%d] Warp [%d] BB [%d] Inst [%d] Trace[%d]",pc,(int)smid,(int)pe_id, (int)warpid,trace_currentbbid, instoffset,traceoffset);
                        //MSG("Thread %d Adding inst to CPU %d, PE %d---->PC %x",(int)active_thread, (int) glofid, (int) pe_id , pc);
                        //}
