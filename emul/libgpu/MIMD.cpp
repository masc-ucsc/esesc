...
bool GPUThreadManager::Timing_MIMD(EmuSampler *gsampler, uint32_t *h_trace_qemu, void *env, uint32_t *qemuid) {
  ...
  do { // for each SM
      ...
      if((SM[smid].smstatus == SMalive) && (SM[smid].timing_warps > 0)) {
        ...
        if(SM[smid].warp_status[warpid] == waiting_barrier) {
          ...
          if(block[blockid].checkThreadCount(blockSize, compl_th) == true) {
            // if (printnow) fprintf (stderr, "SM[%d] Warp [%d] Have to clear warps in block %d :", (int) smid, (int) warpid, (int)
            // blockid);
        ...
        if(SM[smid].warp_status[warpid] == traceReady) {
          ...
          for(int32_t pe_id = SM[smid].warp_last_active_pe[warpid];
              ((pe_id < (int32_t)numSP) || (start_halfway)) && !(tsfifoblocked); pe_id++) {

            glofid = mapLocalID(smid, pe_id);
              ...
              if(SM[smid].thread_status_per_pe[pe_id][warpid] == running) {
                ...
                if(trace_currentbbid == 0) {
                  ...
                  if(kernel->isBarrierBlock(trace_currentbbid)) {
                    ...
                    // if (printnow)
                    // IS(MSG ("BARRIER: SM %d PE %d Thread %d Block %d blocked on BB[%d], will resume from BB[%d]",(int)glofid,
                    // (int)pe_id, (int)active_thread, blockid, (int)trace_currentbbid, (int) trace_nextbbid));
                    ...
                    if(block[blockid].checkThreadCount(blockSize, compl_th) == true) {
                      // if (printnow) fprintf (stderr, "SM[%d] Warp [%d] Have to clear warps in block %d :", (int) smid, (int)
                      // warpid, (int) blockid);
                    ...
                    if(!(kernel->isDummyBB(trace_currentbbid))) {
                      ...
                      if(istsfifoBlocked[glofid]) {
                        ...
                        // if (printnow)
                        // if ((pe_id == 0)) {
                        // MSG("\t\t\t\t\t\t\t\t\t\tThread %d at SM[%d] PE [%d][%llu] BB [%d] Inst [%d]
                        // Trace[%d]",(int)active_thread,(int)smid, (int)pe_id, warpid,trace_currentbbid, instoffset,traceoffset);
                        // MSG("\nEMULATOR: Inserting Instruction PC=%x SM[%d] PE [%d] Warp [%d] BB [%d] Inst [%d]
                        // Trace[%d]",pc,(int)smid,(int)pe_id, (int)warpid,trace_currentbbid, instoffset,traceoffset); MSG("Thread %d
                        // Adding inst to CPU %d, PE %d---->PC %x",(int)active_thread, (int) glofid, (int) pe_id , pc);
                        //}
    ...
    if(newlaunch) {
      for(uint64_t i = 0; i < ret_numSM(); i++) {
        for(uint64_t j = 0; j < ret_numSP(); j++) {
          gsampler->resumeThread(mapLocalID(i, j));
        }
      }
      newlaunch = false;
    }
