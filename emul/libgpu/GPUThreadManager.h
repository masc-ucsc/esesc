      ...
      do {
        ...
        do {
          ...
          if(warp_id < warp_status.size()) {
            fprintf(stderr, "\nWARP ID (%4d) BLOCK ID (%4d) TH (%4d):", warp_id, warp_block_map[warp_id],
...
public:
  ...
  bool canTurnOff(uint64_t smid) { /*{{{*/
    // IS(MSG("timing threads complete = %llu,rabbit threads complete = %llu, totalthreadsin sm =
    // %llu",SM[smid].timingthreadsComplete,SM[smid].rabbitthreadsComplete, SM[smid].totalthreadsinSM));
  ...
  bool canTurnOffTiming(uint64_t smid) { /*{{{*/
    ...
    if((SM[smid].timingthreadsComplete) >= (SM[smid].timing_warps * numSP)) {
      // MSG(" [%d] timing threads complete out of [%d]. Turning it off...",(int)(SM[smid].timingthreadsComplete),
      // (int)(SM[smid].timing_warps*numSP));
      return true;
    }
    //  MSG(" [%d] timing threads complete out of [%d]. Cannot turn it
    //  off...",(int)(SM[smid].timingthreadsComplete),(int)(SM[smid].timing_warps*numSP));
    return false;
  } /*}}}*/
