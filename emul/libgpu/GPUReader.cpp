...
GPUReader::GPUReader(const char *section, EmulInterface *eint_)
  ...
  char *   s = (char *)(SescConf->getCharPtr(section, "params", paramcount - 1));
  ifstream file;
  file.open(s);

  if(!file) {
    cout << "Error in opening benchmark description (*.info) file : " << s << endl;
    exit(-1);
  }
  ...
  while(std::getline(file, lineread)) // Read line by line
  {
    ...
    do {
      ...
      if(lineread.find("Name", 0) != string::npos) {
        ...
        lineread          = lineread.substr(lineread.find("=") + 1, lineread.size());
        kernel->dfl_reg64 = atoi(lineread.c_str());
    ...
    if(!filedone) {
      do {
        ...
        while((lineread.find("[END_KERNEL]") == string::npos) && (lineread.find("[BB]") == string::npos) &&
        ...
        realinstcount = 0;
        int tempint   = 0;
        while((lineread.find("[END_BB]") == string::npos) && (lineread.find("[END_KERNEL]") == string::npos)) {
          // cout<<lineread<<endl;
...
// void GPUReader::queueInstruction(uint32_t insn, AddrType pc, AddrType addr, DataType data,char thumb,  FlowID fid, void *env,
// bool inEmuTiming) {
