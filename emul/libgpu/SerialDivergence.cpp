    ...
    for(smid = 0; smid < numSM; smid++) { // Loop L1
      ...
      if((SM[smid].smstatus == SMalive)) { // Loop L2
          ...
          if(block[blockid].checkThreadCount(blockSize, compl_th) == true) {
            ...
            for(uint32_t w_per_b = 0; w_per_b < block[blockid].warps.size(); w_per_b++) {
              ...
              for(uint32_t spid = 0; spid < numSP; spid++) {
                if((localwid < SM[smid].threads_per_pe[spid].size()) &&
                   (SM[smid].thread_status_per_pe[spid][localwid] != invalid)) {
        ...
        if(SM[smid].warp_status[warpid] == traceReady) { // Loop L3
          ...
          for(int32_t pe_id = SM[smid].warp_last_active_pe[warpid];
              ((pe_id < (int32_t)numSP) || (start_halfway)) && !(tsfifoblocked); pe_id++) { // Loop L4
            if(warpid < SM[smid].threads_per_pe[pe_id].size()) {                            // Loop L5
              ...
              if(SM[smid].thread_status_per_pe[pe_id][warpid] == running) { // Loop L6
                ...
                if(SM[smid].divergence_data_valid[warpid] == false) { // Loop L7
                  ...
                  for(int32_t l_pe_id = 0; (l_pe_id < (int32_t)numSP); l_pe_id++) { // Loop L8
                    ...
                    if((l_pausedbb != 1) && (l_currentbb != 0)) {
                      ...
                      SM[smid].warp_divergence[warpid].bb_mapped_threads.insert(
                          std::pair<uint32_t, uint64_t>(l_currentbb, l_active_thread));
                    } else {
                      if(printnow)
                        MSG("Warp %llu : Ignoring thread %llu which goes to paused basic block %d", warpid, l_active_thread,
                            l_currentbb);
                    }
                  } // End Loop L8
#if 1
                ...
                if(trace_currentbbid == 0) {                                 // Loop L9
                  ...
                  if(trace_currentbbid == SM[smid].unmasked_bb[warpid]) { // Loop L10
                    ...
                    if(kernel->isBarrierBlock(trace_currentbbid)) { // Loop L11
                      ...
                      if(printnow)
                        MSG("Warp %d : BARRIER: SM %d PE %d Thread %d Block %d blocked on BB[%d], will resume from BB[%d]",
                            (int)warpid, (int)glofid, (int)pe_id, (int)active_thread, blockid, (int)trace_currentbbid,
                            (int)trace_nextbbid);
                      ...
                      if(block[blockid].checkThreadCount(blockSize, compl_th) == true) { // Loop L12
                        if(printnow)
                          fprintf(stderr, "Warp %llu : SM[%d] Warp [%d] Have to clear warps in block %d :", warpid, (int)smid,
                                  (int)warpid, (int)blockid);
                      ...
                      if(!(kernel->isDummyBB(trace_currentbbid))) { // Loop L14
                        ...
                        if(op == 0) { // ALU ops, Loop L15
                          ...
                          if((memaccesstype == ParamMem) || (memaccesstype == RegisterMem) || (memaccesstype == ConstantMem) ||
                             (memaccesstype == TextureMem)) {
                        ...
                        if(istsfifoBlocked[glofid]) { // Loop L21
                          if(printnow)
                            MSG("Warp %d :\t\t\t TSFIFO[%llu] BLOCKED, (SM[%u], Warp[%llu]). Moving to Next SM", (int)warpid,
                                glofid, smid, warpid);
                          ...
                          if(printnow)
                            MSG("Warp %d : SM (%d) Thread %d (BB %d ) Adding inst [%d] to CPU %d, PE %d---->PC %x", (int)warpid,
                                (int)smid, (int)active_thread, (int)trace_currentbbid, instoffset, (int)glofid, (int)pe_id, pc);
                          ...
                          if(SM[smid].thread_instoffset_per_pe[pe_id][warpid] >= (int32_t)numbbinst) { // Loop L23
                            ...
                            if(SM[smid].warp_divergence[warpid].bb_mapped_threads.count(trace_currentbbid) == 0) { // Loop L24
                              if(switch2nextDivergentBB(smid, warpid) == true) {
                                ...
                                if(printnow)
                                  cout << "Warp " << warpid << " : All divergent threads in this warp have now been executed."
                                       << endl;
                              }
                            } // End Loop L24
                            ...
                            if(trace_nextbbid == 0) {                                        // Loop L25
                              ...
                              h_trace_qemu[(active_thread * traceSize) + 1] =
                                  0; // Mark the currentbbid to 0 so that u don't keep inserting the instruction
                              ...
                              if(printnow)
                                MSG("Thread %d at SM[%d] PE [%d][%llu] BB [%d] Inst [%d] COMPLETE", (int)active_thread, (int)smid,
                                    (int)pe_id, warpid, trace_currentbbid, instoffset);

                              if(SM[smid].warp_complete[warpid] >= numSP) {
                                if(printnow)
                                  MSG("SM[%d] PE [%d] WARP [%d] COMPLETE.. Should change warp!!", (int)smid, (int)pe_id,
                                      (int)warpid);
                    ...
                    // cout << "Warp " <<warpid<<" : ";
                    if(printnow)
                      cout << "Ignoring thread " << active_thread << " PE (" << pe_id << ") because it goes to BB "
                           << trace_currentbbid << " and not " << SM[smid].unmasked_bb[warpid] << "(Head of divergent bbs is "
                           << *(SM[smid].warp_divergence[warpid].divergent_bbs.begin()) << " )" << endl;
                  } // End Loop L10
                }   // End Loop L9
                ...
                /*
                   ...
                   MSG ("Warp  %llu: Why am I here, in the (If thread_status != running) loop, Thread %llu is running",warpid,
                   active_thread); } else if (SM[smid].thread_status_per_pe[pe_id][warpid] == paused_on_a_memaccess) { MSG ("Warp
                   %llu: Why am I here, in the (If thread_status != running) loop Thread %llu is paused_on_a_memaccess",warpid,
                   active_thread); } else if (SM[smid].thread_status_per_pe[pe_id][warpid] == paused_on_a_barrier) { MSG ("Warp
                   %llu: Why am I here, in the (If thread_status != running) loop Thread %llu is paused_on_a_barrier",warpid,
                   active_thread); } else if (SM[smid].thread_status_per_pe[pe_id][warpid] == block_done) { MSG ("Warp  %llu: Why am
                   I here, in the (If thread_status != running) loop Thread %llu is block_done",warpid, active_thread); } else if
                   (SM[smid].thread_status_per_pe[pe_id][warpid] == invalid) { MSG ("Warp  %llu: Why am I here, in the (If
                   thread_status != running) loop Thread %llu is invalid",warpid, active_thread); } else { MSG ("Warp  %llu: Why am
                   I here, in the (If thread_status != running) loop Thread %llu is weird!!!!",warpid, active_thread);
                   }
                   */
          ...
          if(SM[smid].warp_blocked[warpid] > 0) {                                                                 // Loop L27
            ...
            if(printnow)
              MSG("Warp %llu : [%d] threads in current warp blocked\n [%d] threads in current warp complete\n [%d] threads in "
                  "current warp done\n ",
                  warpid, (int)SM[smid].warp_blocked[warpid], (int)SM[smid].warp_complete[warpid],
                  (int)threads_in_current_warp_done);
          } // End loop L27
          ...
          if(memoryInstfound || changewarpnow) { // Loop L29
            ...
            // See if the new warp has instructions to insert. If yes, then good, otherwise mark SMstatus as SMDone, and be ready
            // for relaunch SM[smid].smstatus = SMdone;
            ...
            if(memoryInstfound) {
              ...
              I((SM[smid].warp_status[warpid] == warpComplete) || (SM[smid].warp_status[warpid] == relaunchReady) ||
                (SM[smid].warp_status[warpid] == waiting_barrier));
            }
...
void GPUThreadManager::printDivergentList(uint64_t smid, uint64_t warpid, uint32_t bbid) {
  fprintf(stderr, "Warp %llu : SM[%llu] contains BB %d (%d elems) : ", warpid, smid, bbid,
          SM[smid].warp_divergence[warpid].bb_mapped_threads.count(bbid));
...
bool GPUThreadManager::switch2nextDivergentBB(uint64_t smid, uint64_t warpid) {
  ...
  do {
    ...
    if(*SM[smid].warp_divergence[warpid].divergent_bbs.begin() != 0) {
      // MSG("Warp %llu : Now adding instructions that go to the next divergent BB
      // (%u)",warpid,*SM[smid].warp_divergence[warpid].divergent_bbs.begin());
      returnvalue = true;
    } else {
      // MSG("Warp %llu : the next divergent BB is zero (%u),
      // Skipping....",warpid,*SM[smid].warp_divergence[warpid].divergent_bbs.begin());
    }
