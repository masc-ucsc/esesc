...
inline void GPUThreadManager::pauseThreadsInRange(uint32_t *h_trace, uint64_t startRange, uint64_t endRange, bool init,
                                                  bool isRabbit) { /*{{{*/
  CUDAKernel *kernel        = kernels[current_CUDA_kernel];
  traceSize               = kernel->tracesize;
  int64_t   active_thread = 0;
...
inline void GPUThreadManager::resumeThreadsInRange(uint32_t *h_trace, uint64_t startRange, uint64_t endRange, bool init,
                                                   bool isRabbit) { /*{{{*/
  CUDAKernel *kernel        = kernels[current_CUDA_kernel];
  traceSize               = kernel->tracesize;
  int64_t   active_thread = 0;
...
inline void GPUThreadManager::resumeSelectivethreads(uint32_t *h_trace, uint32_t smid, uint64_t warpid, bool init,
                                                     bool isRabbit) { /*{{{*/
  int64_t   active_thread = 0;
...
inline void GPUThreadManager::pauseSelectivethreads(uint32_t *h_trace, uint32_t smid, uint64_t warpid, bool init,
                                                    bool isRabbit) { /*{{{*/
  int64_t   active_thread = 0;
  uint64_t  maxcount      = numSP;
...
void GPUThreadManager::reOrganize() { /*{{{*/
  ...
  uint32_tconc_warps = calcOccupancy();
  if(conc_warps == 0) {
    fprintf(stderr, "\n\nThe parameters of the kernel and the specifications of the GPU do not make it possible to launch even a "
                    "single block on the GPGPU\n This kernel cannot be launched\n\n Exiting...\n");
    exit(-1);
  }

  uint32_twarps_per_block = blockSize / warpsize;
  if(blockSize % warpsize > 0)
    warps_per_block++;

  uint32_tblocks_per_sm = conc_warps / warps_per_block;
  uint32_tth_sb_sw      = blocks_per_sm * blockSize;
  ...
  if(maxSamplingThreads > 0) {
    ...
    IS(cout << "maxSamplingThreads rounded to a multiple of " << conc_warps * warpsize << " and is now  " << maxSamplingThreads
            << endl);
  ...
  if(maxSamplingThreads != 0) {
    ...
    if((int64_t)total_timing_threads < 0) {
      ...
      IS(cout << "maxSamplingThreads defined as " << maxSamplingThreads
              << ",which is more than the number of threads in the application ( " << numThreads
              << " ), sampling all the threads... " << endl);
      total_timing_threads = numThreads;
    }
    uint32_t smid = 0;
...
void GPUThreadManager::init_trace(uint32_t *h_trace) { /*{{{*/
  ...
  if(maxSamplingThreads != 0) {
    ...
    for(uint32_t smid = 0; smid < numSM; smid++) {
      if(SM[smid].totalthreadsinSM > 0) {
        IS(fprintf(stderr, "\n\nSM [%u] has %u warps in timing and %u warps fastforwarded", smid, SM[smid].timing_warps,
                   SM[smid].warp_status.size() - SM[smid].timing_warps));
        uint64_t warpid = 0;
...
bool GPUThreadManager::Rabbit(EmuSampler *gsampler, uint32_t *h_trace_qemu, void *env, uint32_t *qemuid) { /*{{{*/
  ...
  uint32_ttrace_currentbbid;
  uint32_ttrace_nextbbid;
  uint32_ttrace_ffwd_insts;
  ...
  // IS(cerr <<"Adding " <<gloicount<< " instructions starting from Basic Block "<<h_trace_qemu[1024*46+1]<< " going to basicBlock "
  // <<h_trace_qemu[1024*46+0]<<endl);
...
bool GPUThreadManager::switch2nextwarp(uint32_t smid, uint32_t *h_trace) { /*{{{*/
  int64_tactive_thread;
  uint32_twarpid = SM[smid].getCurrentWarp();
  ...
  boolwarpfound      = false;
  boolnomoreWarpSets = false;
  uint32_tnewwarpid      = 0;
  uint32_tlocalwarpcount = 0;
  ...
  do {
    ...
    do {
      ...
      */
    } while((SM[smid].warp_status[newwarpid] == warpComplete) || (SM[smid].warp_status[newwarpid] == waiting_barrier));
        ...
        for(uint64_t warpid = SM[smid].timing_warps; warpid < SM[smid].warp_status.size(); warpid++) {
          if(warpid < SM[smid].threads_per_pe[pe_id].size()) {
            if(likely(SM[smid].thread_status_per_pe[pe_id][warpid] != invalid)) {
              ...
              h_trace[(SM[smid].threads_per_pe[pe_id][warpid]) * traceSize + 2] =
                  32767; // Runahead (this is a large number for a single thread)
              h_trace[(SM[smid].threads_per_pe[pe_id][warpid]) * traceSize + 9] =
                  0; // Runahead (this is a large number for a single thread)
            }
          }
        }
      }
...
void GPUThreadManager::decodeSignature(uint32_t *h_trace_qemu) { /*{{{*/
  ofstreamoutdata;                                                    // outdata is like cin
  uint64_tthreadid;                                                   // loop index

  CUDAKernel *kernel   = kernels[current_CUDA_kernel];
  traceSize   = kernel->tracesize;
  stringfilename = kernel->kernel_name;
  filename.append(".csv");
