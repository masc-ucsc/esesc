...
bool GPUThreadManager::Timing_postdom_branchdiv(EmuSampler *gsampler, uint32_t *h_trace_qemu, void *env, uint32_t *qemuid) {
        ...
        if(SM[smid].warp_status[warpid] == traceReady) { // Loop L3
                ...
                if(trace_currentbbid == 0) {                                 // Loop L9
                  ...
                  if(trace_currentbbid == SM[smid].unmasked_bb[warpid]) { // Loop L10
                    ...
                    if(kernel->isBarrierBlock(trace_currentbbid)) { // Loop L11
                      ...
                      // if (printnow)
                      // MSG ("Warp %d : BARRIER: SM %d PE %d Thread %d Block %d blocked on BB[%d], will resume from BB[%d]",(int)
                      // warpid, (int)glofid, (int)pe_id, (int)active_thread, blockid, (int)trace_currentbbid, (int) trace_nextbbid);
                      ...
                      if(!(kernel->isDummyBB(trace_currentbbid))) { // Loop L14
                        ...
                        if(istsfifoBlocked[glofid]) { // Loop L21
                          // MSG("Warp %d :\t\t\t TSFIFO[%llu] BLOCKED, (SM[%u], Warp[%llu]). Moving to Next SM",(int)
                          // warpid,glofid,smid,warpid); MSG("Last Active PE                = %d", (int) pe_id);
          ...
          if(memoryInstfound || changewarpnow) { // Loop L29
            ...
            if(memoryInstfound) {
              ...
              SM[smid].warp_status[warpid] =
                  waiting_memory; // This has to be done at this point, so that all the threads can insert that one instruction
