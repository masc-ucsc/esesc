...
bool GPUThreadManager::Timing_Original_buggy(EmuSampler* gsampler, uint32_t* h_trace_qemu, void* env, uint32_t* qemuid) {
                  ...
                  if (kernel->isBarrierBlock(trace_currentbbid)) {
                    ...
                    if (!(kernel->isDummyBB(trace_currentbbid))) {
                      ...
                      if ((op==2) || (op==1)){
                        ...
                        MSG( "Addr is  = 0x%016X",addr);
                        MSG( "Data is  = 0x%016X",data);
                        uint64_t addr_64bit = pe_id;
                        addr_64bit = addr_64bit << 16 |((SM[smid].getCurrentLocalWarp())*numSP);
                        MSG( "Addr_64bit (with pe and warp information) has  = %llx",addr_64bit);
                        addr_64bit = addr_64bit << 32 | addr;
                        MSG( "Addr_64bit (with addr) has  = %llx",addr_64bit);
                        MSG ("*****************************************************************");
                      }
                      gsampler->queue(insn,pc,addr,glofid,op,icount,env);
