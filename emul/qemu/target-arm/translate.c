...
#define BEGIN_TRACE() \
	op_cnt++; \
  op_cnt_used = 1; \
  int offst = (thumb==1) ? 2 : 4; \
  TCGv_ptr tmp_off = tcg_temp_new_ptr(); \
  TCGv_ptr tmp_pos = tcg_temp_new_ptr(); \
  TCGv tmp_val = tcg_temp_new(); \
	tcg_gen_shli_tl(TCGV_PTR_TO_NAT_GENERIC(tmp_off), cpu_op_cnt, 2);


#define PC_TRACE() \
		tcg_gen_add_ptr(tmp_pos, cpu_op_pc, tmp_off); \
		tcg_gen_movi_tl(tmp_val, pc-offst); \
		tcg_gen_st_tl(tmp_val, tmp_pos, 0); 

   
#define INSN_TRACE(type) \
		tcg_gen_movi_tl(tmp_val, ((thumb << 6) | type)); \
		tcg_gen_add_ptr(tmp_pos, cpu_op_insn, tmp_off); \
		tcg_gen_st_tl(tmp_val, tmp_pos, 0); 

#define ADDR_PTR_TRACE() \
		tcg_gen_add_ptr(tmp_pos, cpu_op_addr, (tmp_off)); \
		tcg_gen_st_tl(addr, tmp_pos, 0);

#define ADDR_MOP_TRACE() \
		tcg_gen_andi_tl(tmp_val, addr, 0xfffffffc); \
		tcg_gen_add_ptr(tmp_pos, cpu_op_addr, (tmp_off)); \
		tcg_gen_st_tl(tmp_val, tmp_pos, 0); 

#define ADDR_TRACE() \
		tcg_gen_movi_tl(tmp_val, addr); \
		tcg_gen_add_ptr(tmp_pos, cpu_op_addr, (tmp_off)); \
		tcg_gen_st_tl(tmp_val, tmp_pos, 0);
...
#define END_TRACE() \
  ...
  tcg_temp_free_ptr(tmp_off); \
  tcg_gen_addi_tl(cpu_op_cnt, cpu_op_cnt, 1);

#define BEGIN_POSTTRACE() \
  TCGv_ptr tmp_off = tcg_temp_new_ptr(); \
  TCGv_ptr tmp_pos = tcg_temp_new_ptr(); \
  TCGv tmp_val = tcg_temp_new(); \
  tcg_gen_subi_tl(tmp_val, cpu_op_cnt, 1); \
	tcg_gen_shli_tl(TCGV_PTR_TO_NAT_GENERIC(tmp_off), tmp_val, 2);

#define END_POSTTRACE() \
	tcg_temp_free(tmp_val); \
  tcg_temp_free_ptr(tmp_pos); \
  tcg_temp_free_ptr(tmp_off); 

/* Allocate a temporary variable.  */
static inline void tcg_gen_trace_postmisc(int thumb)
{
	BEGIN_POSTTRACE();
...
static inline void tcg_gen_trace_misc(int thumb, uint32_t pc)
{
  if (prectrl_called) {
    tcg_gen_trace_postmisc(thumb);
    return;
  }
	BEGIN_TRACE();
...
static inline void tcg_gen_trace_prectrl(int thumb, uint32_t pc)
{
	BEGIN_TRACE();
...
static inline void tcg_gen_trace_postctrl_ptr(TCGv addr)
{
	BEGIN_POSTTRACE();
...
static inline void tcg_gen_trace_postctrl(uint32_t addr)
{
	BEGIN_POSTTRACE();
...
static inline void tcg_gen_trace_ctrl_ptr(int thumb, uint32_t pc, TCGv addr)
{
  if (prectrl_called) {
    tcg_gen_trace_postctrl_ptr(addr);
    return;
  }
	BEGIN_TRACE();
...
static inline void tcg_gen_trace_ctrl(int thumb, uint32_t pc, uint32_t addr)
{
  if (prectrl_called) {
    tcg_gen_trace_postctrl(addr);
    return;
  }
	BEGIN_TRACE();
...
static inline void tcg_gen_trace_postld(int thumb, TCGv addr, int index)
{
	BEGIN_POSTTRACE();
...
static inline void tcg_gen_trace_ld(int thumb, uint32_t pc, TCGv addr, int index)
{
  if (prectrl_called) {
    tcg_gen_trace_postld(thumb,addr,index);
    return;
  }
...
static inline void tcg_gen_trace_postst(int thumb,TCGv addr, int index)
{
	BEGIN_POSTTRACE();
...
static inline void tcg_gen_trace_st(int thumb, uint32_t pc, TCGv addr, int index)
{
  if (prectrl_called) {
    tcg_gen_trace_postst(thumb,addr,index);
    return;
  }
	BEGIN_TRACE();
...
static inline void tcg_gen_trace_ctrl(int thumb, uint32_t pc, TCGv addr)
{
}
static inline void tcg_gen_trace_ld(int thumb, uint32_t pc, TCGv addr, int index)
{
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
  tcg_gen_trace_ctrl_ptr(thumb, s->pc, var);
#endif
	tcg_gen_andi_i32(var, var, 1);
...
{
  TCGv tmp = tcg_temp_new_i32();
  tcg_gen_trace_ld(thumb, pc, addr, index);
...
static inline void gen_st8(int thumb, uint32_t pc, TCGv val, TCGv addr, int index)
{
  ...
  tcg_gen_trace_st(thumb, pc, addr, index);
...
{
  tcg_gen_trace_ld(thumb, s->pc, addr, IS_USER(s));
  if (dp)
...
static inline void gen_vfp_st(int thumb, DisasContext *s, int dp, TCGv addr)
{
  ...
  tcg_gen_trace_st(thumb, s->pc, addr, IS_USER(s));
}
#else
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
    if (op_cnt_used==0) //{
      tcg_gen_trace_misc(thumb, dc->pc);
      //}else{
      op_cnt_used = 0;
      //}
#endif
