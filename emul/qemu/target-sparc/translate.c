...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
static TCGv_i32 cpu_last_addr;
static TCGv_i32 cpu_last_data;
static TCGv_i32 cpu_last_op;
#endif
            ...
            case 0x2:           /* BN+x */
                {
                    ...
                    do_branch(dc, target, insn, 0, cpu_cond);
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
                    tcg_gen_movi_i32(cpu_last_op, 3); // BALU_BRANCH
#endif
                    goto jmp_insn;
                }
            case 0x6:           /* FBN+x */
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
                    tcg_gen_mov_i32(cpu_last_addr, cpu_addr);
                    tcg_gen_mov_i32(cpu_last_data, cpu_val);
                    tcg_gen_movi_i32(cpu_last_op, 1); // LD
										dc->is_mem = 1;
#endif
                    break;
                ...
                case 0xd:       /* ldstub -- XXX: should be atomically */
                    {
                        ...
                        tcg_gen_mov_i32(cpu_last_addr, cpu_addr);
                        tcg_gen_mov_i32(cpu_last_data, cpu_val);
                        tcg_gen_movi_i32(cpu_last_op, 2); // ST?
												dc->is_mem = 1;
#endif
                    }
                    break;
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
                    tcg_gen_mov_i32(cpu_last_addr, cpu_addr);
                    tcg_gen_mov_i32(cpu_last_data, cpu_tmp0);
                    tcg_gen_movi_i32(cpu_last_op, 1); // LD
										dc->is_mem = 1;
#endif
                    break;
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
                    tcg_gen_mov_i32(cpu_last_addr, cpu_addr);
                    tcg_gen_mov_i32(cpu_last_data, cpu_tmp32);
                    tcg_gen_movi_i32(cpu_last_op, 1); // LD
										dc->is_mem = 1;
#endif
                    }
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
                        tcg_gen_mov_i32(cpu_last_addr, cpu_addr);
                        tcg_gen_mov_i32(cpu_last_data, cpu_fpr[rd]);
                        tcg_gen_movi_i32(cpu_last_op, 1); // LD
												dc->is_mem = 1;
#endif
                    break;
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
                        tcg_gen_mov_i32(cpu_last_addr, cpu_addr);
                        tcg_gen_mov_i32(cpu_last_data, r_const);
                        tcg_gen_movi_i32(cpu_last_op, 2); // ST
												dc->is_mem = 1;
#endif
                    break;
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
				if (dc->is_mem && !esesc_allow_large_tb)
					break;
#endif
        if (dc->is_br)
            break;
...
#if defined (CONFIG_ESESC_system) || defined (CONFIG_ESESC_user)
        cpu_last_addr = tcg_global_mem_new_i32(TCG_AREG0,
            offsetof(CPUState, last_addr), "last_addr");
        cpu_last_data = tcg_global_mem_new_i32(TCG_AREG0,
            offsetof(CPUState, last_data), "last_data");
        cpu_last_op = tcg_global_mem_new_i32(TCG_AREG0,
            offsetof(CPUState, last_op), "last_op");
#endif
        /* register helpers */
